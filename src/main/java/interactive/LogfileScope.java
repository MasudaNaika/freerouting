...
/**
 * Enumeration class defining scopes in a logfile, Each Object of the class must
 * implement the read_scope method.
 *
 * @author Alfons Wirtz
 */
public abstract class LogfileScope {
    ...
    private static abstract class CornerlistScope extends LogfileScope {
        ...
        /**
         * Reads the next corner list scope togethet with its interiour scopes
         * (layer change for example) from the input logfile. Returns the active
         * interactive state after reading the scope.
         */
        @Override
        public InteractiveState read_scope(Logfile p_logfile,
            ...
            InteractiveState interactive_state
                    = start_scope(location, p_return_state, p_board_handling);
    ...
    /**
     * Scope calling the single method
     * SelectedItemState.assign_items_to_new_group
     */
    ...
    private static class ZoomFrameScope extends LogfileScope {
        ...
        public InteractiveState read_scope(Logfile p_logfile, InteractiveState p_return_state, BoardHandling p_board_handling) {
            ...
            Point2D upper_right = p_board_handling.graphics_context.coordinate_transform.board_to_screen(p_logfile.read_corner());
