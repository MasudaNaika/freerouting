...
public class SelectedItemState extends InteractiveState {
    ...
    /**
     * Creates a new SelectedItemState with with the items in p_item_list
     * selected. Returns null, if p_item_list is empty.
     */
        ...
        switch (p_key_char) {
            ...
            case 'm':
                result = MoveItemState.get_instance(hdlg.get_current_mouse_position(), item_list,
                        return_state, hdlg, logfile);
                break;
            case 'n':
                extent_to_whole_nets();
                break;
            case 'p':
                hdlg.optimize_selected_items();
                break;
            case 'r':
    ...
    /**
     * Makes all items in this selected_set connectable and assigns them to a
     * new net.
     */
    ...
    /**
     * Deletes all unfixed items in this selected set and pulls tight the
     * neighour traces.
     */
    ...
    public InteractiveState cutout_items() {
        return CutoutRouteState.get_instance(item_list, return_state, hdlg, logfile);
    }

    /**
     * Autoroutes the selected items. If p_stoppable_thread != null, the
     * algorithm can be requestet to terminate.
     */
     ...
     * != null, the algorithm can be requestet to terminate.
     */
    ...
    /**
     * Optimizes the selected items. If p_stoppable_thread != null, the
     * algorithm can be requestet to terminate.
     */
    ...
    /**
     * Select also all items belonging to any group of the current selected
     * items.
     */
    ...
    /**
     * Select also all items belonging to any connected set of the current
     * selected items.
     */
     ...
     * already in there, otherwise adds it to the list. Returns true (to change
     * to the return_state) if nothing was picked.
     */
    public InteractiveState toggle_select(FloatPoint p_point) {
        ...
        if (picked_items.size() == 1) {
            ...
            if (item_list.contains(picked_item)) {
                item_list.remove(picked_item);
                if (item_list.isEmpty()) {
                    state_ended = true;
                }
            } else {
                item_list.add(picked_item);
            }
        }
        hdlg.repaint();
    ...
    public SelectedItemState info() {
        gui.WindowObjectInfo.display(item_list, hdlg.get_panel().board_frame, hdlg.coordinate_transform, new java.awt.Point(100, 100));
        return this;
    }
