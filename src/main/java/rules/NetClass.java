 ...
 * @author Alfons Wirtz
 */
public class NetClass implements Serializable, board.ObjectInfoPanel.Printable {
    ...
    public NetClass(String p_name, board.LayerStructure p_layer_structure, ClearanceMatrix p_clearance_matrix) {
        ...
        trace_half_width_arr = new int[p_layer_structure.arr.length];
        active_routing_layer_arr = new boolean[p_layer_structure.arr.length];
        for (int i = 0; i < p_layer_structure.arr.length; ++i) {
            active_routing_layer_arr[i] = p_layer_structure.arr[i].is_signal;
        }
    ...
    /**
     * Sets the trace half width used for routing to p_value on all inner
     * layers.
     */
    ...
    /**
     * Sets, if the cycle remove algorithm ignores cycles, where conduction
     * areas are involved
     */
    ...
    /**
     * Returns, if the cycle remove algorithm ignores cycles, where conduction
     * areas are involved
     */
    ...
    /**
     * Returns the minimum trace length of this net class. If the result is <=
    ...
    /**
     * Sets the minimum trace length of this net class to p_value. If p_value is
     * <= 0, there is no minimal trace length restriction.
     */
    ...
    /**
     * Returns the maximum trace length of this net class. If the result is <=
    ...
    /**
     * Sets the maximum trace length of this net class to p_value. If p_value is
     * <= 0, there is no maximal trace length restriction.
     */
    ...
    public void set_all_layers_active(boolean p_value) {
        Arrays.fill(active_routing_layer_arr, p_value);
    }
    ...
    public void print_info(board.ObjectInfoPanel p_window, Locale p_locale) {
        ...
        if (trace_width_is_layer_dependent()) {
            for (int i = 0; i < trace_half_width_arr.length; ++i) {
                ...
                p_window.append(board_layer_structure.arr[i].name);
            }
        } else {
            p_window.append(", " + resources.getString("trace_width") + " ");
    ...
    /**
     * Returns true, if the trace width of this class is not equal on all
     * layers.
     */
    public boolean trace_width_is_layer_dependent() {
        ...
        for (int i = 1; i < trace_half_width_arr.length; ++i) {
            if (board_layer_structure.arr[i].is_signal) {
    ...
    /**
     * Returns true, if the trace width of this class is not equal on all inner
     * layers.
     */
    public boolean trace_width_is_inner_layer_dependent() {
        ...
        while (!board_layer_structure.arr[first_inner_layer_no].is_signal) {
