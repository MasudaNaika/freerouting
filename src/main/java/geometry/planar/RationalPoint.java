...
/**
 *
 * Implementation of points in the projective plane represented by 3 coordinates
 * x, y, z, which are infinite precision integers. Two projective points (x1,
 * y1, z1) and (x2, y2 z2) are equal, if they are located on the same line
 * through the zero point, that means, there exist a number r with x2 = r*x1, y2
 * = r*y1 and z2 = r*z1. The affine Point with rational coordinates represented
 * by the projective Point (x, y, z) is (x/z, y/z). The projective plane with
 * integer coordinates contains in addition to the affine plane with rational
 * coordinates the so-called line at infinity, which consist of all projective
...
public class RationalPoint extends Point implements Serializable {
     ...
     * right f the line from p_1 to p_2; and Side.COLLINEAR, if this Point is
    ...
    int compare_x(RationalPoint p_other) {
        BigInteger tmp1 = x.multiply(p_other.z);
        BigInteger tmp2 = p_other.x.multiply(z);
        return tmp1.compareTo(tmp2);
    }

    @Override
    int compare_y(RationalPoint p_other) {
        BigInteger tmp1 = y.multiply(p_other.z);
        BigInteger tmp2 = p_other.y.multiply(z);
        return tmp1.compareTo(tmp2);
    }

    @Override
    int compare_x(IntPoint p_other) {
        BigInteger tmp1 = z.multiply(BigInteger.valueOf(p_other.x));
        return x.compareTo(tmp1);
    }

    @Override
    int compare_y(IntPoint p_other) {
        BigInteger tmp1 = z.multiply(BigInteger.valueOf(p_other.y));
        return y.compareTo(tmp1);
