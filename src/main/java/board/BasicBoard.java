...
/**
 *
 * Provides basic functionality of a board with geometric items. Contains
 * functions such as inserting, deleting, modifying and picking items and
 * elementary checking functions. A board may have 1 or several layers.
 *
 * @author Alfons Wirtz
 */
public class BasicBoard implements Serializable {
     ...
     * is used for syncronisation, if the board is generated by a host database.
     * Otherwise it is null. If p_test_level != RELEASE_VERSION,, some features
    ...
    /**
     * Inserts a trace into the board, whose geometry is described by a
     * Polyline. p_clearance_class is the index in the clearance_matix, which
     * describes the required clearance restrictions to other items. Because no
     * internal cleaning of items is done, the new inserted item can be
     * returned.
     */
     ...
     * Inserts a trace into the board, whose geometry is described by a
     * Polyline. p_clearance_class is the index in the clearance_matix, which
     * describes the required clearance restrictions to other items.
     */
    ...
    /**
     * Inserts a trace into the board, whose geometry is described by an array
     * of points, and cleans up the net.
     */
     ...
     * polygonyal shape, which may have holes. If p_component_no != 0, the
     * obstacle belongs to a component.
     */
     ...
     * identify the corresponding ObstacstacleArea in the component package.
     */
    ...
    /**
     * Inserts an via obstacle belonging to a component into the board p_name is
     * to identify the corresponding ObstacstacleArea in the component package.
     */
     ...
     * described by a polygonyal shape, which may have holes.
     */
     ...
     * p_name is to identify the corresponding ObstacstacleArea in the component
     * package.
     */
    ...
    /**
     * Inserts a condution area into the board , whose geometry is described by
     * a polygonyal shape, which may have holes. If p_is_obstacle is false, it
     * is possible to route through the conduction area with traces and vias of
     * foreign nets.
     */
    ...
    /**
     * Returns the pin with the input component number and pin number, or null,
     * if no such pinn exists.
     */
    ...
    /**
     * Removes the items in p_item_list. Returns false, if some items could not
     * be removed, bcause they are fixed.
     */
    ...
    /**
     * Combines the connected traces of this net, which have only 1 contact at
     * the connection point. if p_net_no < 0 traces of all nets are combined.
     */
    ...
    /**
     * Looks for traces of the input net on the input layer, so that p_location
     * is on the trace polygon, and splits these traces. Returns false, if no
     * trace was split.
     */
    public boolean split_traces(Point p_location, int p_layer, int p_net_no) {
        ...
        Collection<Item> picked_items = pick_items(p_location, p_layer, filter);
    ...
    public Set<SearchTreeObject> overlapping_objects(ConvexShape p_shape, int p_layer) {
        return search_tree_manager.get_default_tree().overlapping_objects(p_shape, p_layer);
    }
    ...
    /**
     * Checks, if the an object with shape p_shape and net nos p_net_no_arr and
     * clearance class p_cl_class can be inserted on layer p_layer without
     * clearance violation.
     */
    ...
    /**
     * Checks, if the a trace line with shape p_shape and net numbers
     * p_net_no_arr and clearance class p_cl_class can be inserted on layer
     * p_layer without clearance violation. If p_contact_pins != null, all pins
     * not contained in p_contact_pins are regarded as obstacles, even if they
     * are of the own net.
     */
    ...
    public boolean check_polyline_trace(Polyline p_polyline, int p_layer, int p_pen_half_width,
        ...
        for (int i = 0; i < tmp_trace.tile_shape_count(); ++i) {
            if (!check_trace_shape(tmp_trace.get_tile_shape(i), p_layer, p_net_no_arr,
     ...
     * overwritten paintComponent method of a class derived from JPanel. The
     * value of p_layer_visibility is expected between 0 and 1 for each layer.
     */
    ...
    /**
     * Returns the list of items on the board, whose shape on layer p_layer
     * contains the point at p_location. If p_layer < 0, the layer is ignored.
     * If p_item_selection_filter != null, only items of types selected by the
     * filter are picked.
     */
     ...
     * p_class_1 and p_class_2. p_class_1 and p_class_2 are indices in the
     * clearance matrix.
     */
    ...
    /**
     * Turns an obstacle area into a conduction area with net number p_net_no If
     * it is convex and has no holes, it is turned into a Pin, alse into a
     * conduction area.
     */
    ...
    /**
     * Stub function overwritten in class RoutingBoard to maintain the
     * autorouter database if necessesary.
     */
    ...
    public boolean undo(IntSortedSet p_changed_nets) {
        ...
        for (UndoableObjects.Storable uos : cancelled_objects) {
            Item curr_item = (Item) uos;
        ...
        for (UndoableObjects.Storable uos : restored_objects) {
            Item curr_item = (Item) uos;
     ...
     * which has no normal contact at that position. Returns null, if no tail is
     * found.
     */
    ...
    /**
     * Checks, if p_item item is part of a cycle and remuve it together with its
    ...
    /**
     * If != RELEASE_VERSION,, some features may be used, which are still in
     * experimental state. Also warnings for debugging may be printed depending
     * on the test_level.
     */
