...
/**
 *
 * An item on the board with an relative_area shape, for example keepout,
 * conduction relative_area
 *
 *
 *
 * @author Alfons Wirtz
...
public class ObstacleArea extends Item implements Serializable {
     ...
     * is null, if the ObstacleArea does not belong to a component.
     */
    ...
    /**
     * Creates a new relative_area item without net. p_name is null, if the
    ...
    public Area get_area() {
        if (precalculated_absolute_area == null) {
            ...
            if (side_changed && !board.components.get_flip_style_rotate_first()) {
    ...
    public void change_placement_side(IntPoint p_pole) {
        ...
        if (board != null) {
            layer = board.get_layer_count() - layer - 1;
        }
        Point rel_location = Point.ZERO.translate_by(translation);
    ...
    public void draw(Graphics p_g, GraphicsContext p_graphics_context, Color[] p_color_arr, double p_intensity) {
        ...
        double intensity = p_graphics_context.get_layer_visibility(layer) * p_intensity;
    ...
    protected final void print_shape_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        p_window.append(board.layer_structure.arr[get_layer()].name);
    }
