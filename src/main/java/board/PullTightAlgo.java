...
public abstract class PullTightAlgo {
     ...
     * traces with net number p_not_no are optimized. If p_stoppable_thread !=
     * null, the agorithm can be requested to be stopped. If p_time_limit > 0;
     * the algorithm will be stopped after p_time_limit Milliseconds.
     */
    ...
    PullTightAlgo(RoutingBoard p_board, int[] p_only_net_no_arr, Stoppable p_stoppable_thread, int p_time_limit,
        ...
        if (p_time_limit > 0) {
            time_limit = new TimeLimit(p_time_limit);
        } else {
            time_limit = null;
        }
        keep_point = p_keep_point;
        keep_point_layer = p_keep_point_layer;
    }

    /**
     * Function for optimizing the route in an internal marked area. If
     * p_clip_shape != null, the optimizing area is restricted to p_clip_shape.
            ...
            for (int i = 0; i < board.get_layer_count(); ++i) {
                ...
                for (SearchTreeObject curr_ob : items) {
                    ...
                    if (curr_ob instanceof PolylineTrace) {
                        ...
                        if (OptViaAlgo.opt_via_location(board, (Via) curr_ob,
                                p_trace_cost_arr, min_translate_dist, 10)) {
                            something_changed = true;
                        }
                    }
                }
    ...
    /**
     * Function for optimizing a single trace polygon p_contact_pins are the
     * pins at the end corners of p_polyline. Other pins are regarded as
    ...
    protected boolean is_stop_requested() {
        if (stoppable_thread != null && stoppable_thread.is_stop_requested()) {
            return true;
        }
        if (time_limit == null) {
            return false;
    ...
    /**
     * Tries to reposition the line with index p_no to make the polyline
     * consisting of p_line_arr shorter.
     */
        ...
        while (first_time || Math.abs(delta_dist) > min_translate_dist) {
            ...
            if (tmp.arr.length == 3) {
                ...
                check_ok = board.check_trace_shape(shape_to_check,
                        curr_layer, curr_net_no_arr, curr_cl_type, contact_pins);
    ...
    /**
     * tries to skip linesegments of length 0. A check is nessesary before
     * skipping because new dog ears may occur.
     */
    Polyline skip_segments_of_length_0(Polyline p_polyline) {
        ...
        for (int i = 1; i < curr_polyline.arr.length - 1; ++i) {
            boolean try_skip;
            if (i == 1 || i == curr_polyline.arr.length - 2) // the position of the first corner and the last corner
    ...
    /**
     * Smoothens acute angles with contact traces. Returns true, if something
     * was changed.
     */
    ...
    private boolean smoothen_end_corners_at_trace_1(PolylineTrace p_trace) {
        ...
        Set<Pin> saved_contact_pins = contact_pins;
    ...
    boolean split_traces_at_keep_point() {
        ...
        Collection<Item> picked_items = board.pick_items(keep_point, keep_point_layer, filter);
        for (Item curr_item : picked_items) {
            Trace[] split_pieces = ((Trace) curr_item).split(keep_point);
    ...
    /**
     * Smoothens acute angles with contact traces. Returns null, if something
     * was changed.
     */
    ...
    protected Polyline avoid_acid_traps(Polyline p_polyline) {
        ...
        if (new_polyline != null && new_polyline != p_polyline) {
            if (board.check_polyline_trace(new_polyline, curr_layer, curr_half_width,
