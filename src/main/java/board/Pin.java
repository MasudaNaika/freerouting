...
/**
 * Class describing the functionality of an electrical Item on the board with a
 * shape on 1 or several layers.
 *
 * @author Alfons Wirtz
 */
public class Pin extends DrillItem implements Serializable {
     ...
     * p_from_layer + 1) shapes must be provided. p_pin_no isthe number of the
     * pin in its component (starting with 0).
     */
    ...
    public Item copy(int p_id_no) {
        ...
        return new Pin(get_component_no(), pin_no, curr_net_no_arr, clearance_class_no(),
    ...
    public Shape get_shape(int p_index) {
        ...
        if (precalculated_shapes == null) {
            ...
            for (int index = 0; index < precalculated_shapes.length; ++index) {
                ...
                precalculated_shapes[index] = (ConvexShape) translated_shape.translate_by(component_translation);
            }
    ...
    /**
     * Returns the layer of the padstack shape corresponding to the shape with
     * index p_index.
     */
    ...
    /**
     * Calculates the allowed trace exit directions of the shape of this
     * padstack on layer p_layer together with the minimal trace line lengths
     * into thei directions. Currently only implemented only for box shapes,
     * where traces are allowed to exit the pad only on the small sides.
     */
    public Collection<TraceExitRestriction> get_trace_exit_restrictions(int p_layer) {
        ...
        Collection<Direction> padstack_exit_directions
                = get_padstack().get_trace_exit_directions(padstack_layer, pad_xy_factor);
    ...
    /**
     * Returns true, if vias throw the pads of this pins are allowed, false,
     * otherwise. Currently drills are allowed to SMD-pins.
     */
    ...
    public boolean is_obstacle(Item p_other) {
        ...
        if (drill_allowed() && p_other instanceof Via && ((Via) p_other).attach_allowed) {
            return false;
        }
        return true;
    }
    ...
    public Set<Pin> get_swappable_pins() {
        ...
        library.LogicalPart.PartPin this_part_pin = logical_part.get_pin(pin_no);
        ...
        for (int i = 0; i < logical_part.pin_count(); ++i) {
            ...
            if (curr_part_pin != null && curr_part_pin.gate_pin_swap_code == this_part_pin.gate_pin_swap_code
                ...
                Pin curr_swappeble_pin = board.get_pin(get_component_no(), curr_part_pin.pin_no);
    ...
    /**
     * Returns the pin, this pin was changed to by pin swapping, or this pin, if
     * it was not swapped.
     */
    ...
    /**
     * Returns the neckdown half width for traces on p_layer. The neckdown width
     * is used, when the pin width is smmaller than the trace width to enter or
     * leave the pin with a trace.
     */
    ...
    public void print_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        library.Padstack padstack = get_padstack();
    ...
    /**
     * Calculates the nearest exit restriction direction for changing
     * p_trace_polyline. p_trace_polyline is assumed to start at the pin center.
    ...
    Direction calc_nearest_exit_restriction_direction(Polyline p_trace_polyline, int p_trace_half_width, int p_layer) {
        Collection<Pin.TraceExitRestriction> trace_exit_restrictions = get_trace_exit_restrictions(p_layer);
        if (trace_exit_restrictions.isEmpty()) {
            return null;
        }
        Shape pin_shape = get_shape(p_layer - first_layer());
        Point pin_center = get_center();
    ...
    /**
     * Calculates the nearest trace exit point of the pin on p_layer. Returns
     * null, if the pin has no trace exit restrictions.
     */
