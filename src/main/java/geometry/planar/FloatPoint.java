...
/**
 *
 * Implements a point in the plane as a touple of double's. Because arithmetic
 * calculations with double's are in general not exact, FloatPoint is not
 * derived from the abstract class Point.
 *
 *
 * @author Alfons Wirtz
 */
public class FloatPoint implements Serializable {
    ...
    public double weighted_distance(FloatPoint p_other, double p_horizontal_weight, double p_vertical_weight) {
        double delta_x = x - p_other.x;
        double delta_y = y - p_other.y;
    ...
    /**
     * rounds the coordinates from an object of class Point_double to an object
     * of class IntPoint
     */
    ...
    public IntPoint round_to_grid(int p_horizontal_grid, int p_vertical_grid) {
        ...
        if (p_horizontal_grid > 0) {
            rounded_x = Math.rint(x / p_horizontal_grid) * p_horizontal_grid;
        } else {
            rounded_x = x;
        }
        double rounded_y;
        if (p_vertical_grid > 0) {
            rounded_y = Math.rint(y / p_vertical_grid) * p_vertical_grid;
        } else {
            rounded_y = y;
        }
    ...
    public FloatPoint add(FloatPoint p_other) {
        return new FloatPoint(x + p_other.x, y + p_other.y);
    }
    ...
    /**
     * Returns an approximation of the perpendicular projection of this point
     * onto p_line
     */
     ...
     * distance p_new_length from zero.
     */
    ...
    /**
     * Approximates a FloatPoint on the line from this point to p_to_point with
     * distance p_new_length from this point.
     */
    public FloatPoint change_length(FloatPoint p_to_point, double p_new_length) {
        ...
        double new_x = x + (dx * p_new_length) / length;
        double new_y = y + (dy * p_new_length) / length;
        return new FloatPoint(new_x, new_y);
    }
    ...
    public FloatPoint middle_point(FloatPoint p_to_point) {
        ...
        double middle_x = 0.5 * (x + p_to_point.x);
        double middle_y = 0.5 * (y + p_to_point.y);
        return new FloatPoint(middle_x, middle_y);
    }

    /**
     * The function returns Side.ON_THE_LEFT, if this Point is on the left of
     * the line from p_1 to p_2; and Side.ON_THE_RIGHT, if this Point is on the
     * right of the line from p_1 to p_2. Collinearity is not defined, becouse
     * numerical calculations ar not exact for FloatPoints.
     */
    public Side side_of(FloatPoint p_1, FloatPoint p_2) {
        double d21_x = p_2.x - p_1.x;
        double d21_y = p_2.y - p_1.y;
        double d01_x = x - p_1.x;
        double d01_y = y - p_1.y;
    ...
    public FloatPoint turn_90_degree(int p_factor, FloatPoint p_pole) {
        FloatPoint v = substract(p_pole);
        v = v.turn_90_degree(p_factor);
        return p_pole.add(v);
    }

    /**
     * Checks, if this point is contained in the box spanned by p_1 and p_2 with
     * the input tolerance.
     */
    public boolean is_contained_in_box(FloatPoint p_1, FloatPoint p_2, double p_tolerance) {
        ...
        if (x < min_x - p_tolerance || x > max_x + p_tolerance) {
            return false;
        }
        double min_y;
        double max_y;
        ...
        return (y >= min_y - p_tolerance && y <= max_y + p_tolerance);
    }
    ...
    public IntBox bounding_box() {
        IntPoint lower_left = new IntPoint((int) Math.floor(x), (int) Math.floor(y));
        IntPoint upper_right = new IntPoint((int) Math.ceil(x), (int) Math.ceil(y));
        return new IntBox(lower_left, upper_right);
    }

    /**
     * Calculates the touching points of the tangents from this point to a
     * circle around p_to_point with radius p_distance. Solves the quadratic
     * equation, which results by substituting x by the term in y from the
     * equation of the polar line of a circle with center p_to_point and radius
     * p_distance and putting it into the circle equation. The polar line is the
     * line through the 2 tangential points of the circle looked at from from
     * this point and has the equation (this.x - p_to_point.x) * (x -
     * p_to_point.x) + (this.y - p_to_point.y) * (y - p_to_point.y) = p_distance
     * **2
     */
    public FloatPoint[] tangential_points(FloatPoint p_to_point, double p_distance) {
        ...
        double dx = Math.abs(x - p_to_point.x);
        double dy = Math.abs(y - p_to_point.y);
    ...
    /**
     * Calculates the left tangential point of the line from this point to a
     * circle around p_to_point with radius p_distance. Returns null, if this
     * point is inside this circle.
     */
    ...
    /**
     * Calculates the right tangential point of the line from this point to a
     * circle around p_to_point with radius p_distance. Returns null, if this
     * point is inside this circle.
     */
    ...
    /**
     * Calculates the center of the circle through this point, p_1 and p_2 by
     * calculating the intersection of the two lines perpendicular to and
     * passing through the midpoints of the lines (this, p_1) and (p_1, p_2).
     */
    public FloatPoint circle_center(FloatPoint p_1, FloatPoint p_2) {
        double slope_1 = (p_1.y - y) / (p_1.x - x);
        double slope_2 = (p_2.y - p_1.y) / (p_2.x - p_1.x);
        double x_center
                = (slope_1 * slope_2 * (y - p_2.y) + slope_2 * (x + p_1.x) - slope_1 * (p_1.x + p_2.x))
                / (2 * (slope_2 - slope_1));
        double y_center = (0.5 * (x + p_1.x) - x_center) / slope_1 + 0.5 * (y + p_1.y);
        return new FloatPoint(x_center, y_center);
    }

    /**
     * Returns true, if this point is contained in the circle through p_1, p_2
     * and p_3.
     */
