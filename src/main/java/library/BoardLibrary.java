...
public class BoardLibrary implements Serializable {
    ...
    /**
     * Sets the subset of padstacks from this.padstacks, which can be used in
    ...
    public Padstack get_via_padstack(int p_no) {
        if (via_padstacks == null || p_no < 0 || p_no >= via_padstacks.size()) {
            return null;
        }
    ...
    /**
     * Gets the via padstack with name p_name, or null, if no such padstack
     * exists.
     */
    public Padstack get_via_padstack(String p_name) {
        ...
        for (Padstack curr_padstack : via_padstacks) {
    ...
    /**
     * Apppends p_padstack to the list of via padstacks. Returns false, if the
     * list contains already a padstack with p_padstack.name.
     */
    ...
    /**
     * Removes p_padstack from the via padstack list. Returns false, if
     * p_padstack was not found in the list. If the padstack is no more used on
     * the board, it will also be removed from the board padstacks.
     */
    ...
    /**
     * Gets the via padstack mirrored to the back side of the board. Returns
     * null, if no such via padstack exists.
     */
    public Padstack get_mirrored_via_padstack(Padstack p_via_padstack) {
        int layer_count = padstacks.board_layer_structure.arr.length;
    ...
    public boolean is_used(Padstack p_padstack, board.BasicBoard p_board) {
        Iterator<datastructures.UndoableObjects.UndoableObjectNode> it = p_board.item_list.start_read_object();
        while (true) {
        ...
        for (int i = 1; i <= packages.count(); ++i) {
            Package curr_package = packages.get(i);
