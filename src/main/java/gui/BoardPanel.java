...
public class BoardPanel extends JPanel {
    ...
    public BoardPanel(ScreenMessages p_screen_messages, BoardFrame p_board_frame,
            boolean p_is_web_application, Locale p_locale) {
    ...
    void create_popup_menus() {
        popup_menu_main = new PopupMenuMain(board_frame);
        popup_menu_dynamic_route = new PopupMenuDynamicRoute(board_frame);
        popup_menu_stitch_route = new PopupMenuStitchRoute(board_frame);
        popup_menu_corneritem_construction = new PupupMenuCornerItemConstruction(board_frame);
        popup_menu_select = new PopupMenuSelectedItems(board_frame);
        popup_menu_insert_cancel = new PopupMenuInsertCancel(board_frame);
        popup_menu_copy = new PopupMenuCopy(board_frame);
        popup_menu_move = new PopupMenuMove(board_frame);
    }
    ...
    public void center_display(Point2D p_new_center) {
        ...
        repaint();
        board_handling.logfile.start_scope(interactive.LogfileScope.CENTER_DISPLAY);
        geometry.planar.FloatPoint curr_corner = new geometry.planar.FloatPoint(p_new_center.getX(), p_new_center.getY());
        board_handling.logfile.add_corner(curr_corner);
    }
    ...
    /**
     * zooms the content of the board by p_factor Returns the change of the
     * cursor location
     */
    ...
    /**
     * Sets the viewport center to p_point. Adjust the result, if p_point is
     * near the border of the viewport. Returns the adjustment vector
     */
    java.awt.Point set_viewport_center(Point2D p_point) {
        java.awt.Rectangle display_rect = get_viewport_bounds();
    ...
    public void set_selected_signal_layer(int p_signal_layer_no) {
        if (board_frame.select_parameter_window != null) {
            board_frame.select_parameter_window.select(p_signal_layer_no);
    ...
    private void scroll_middle_mouse(MouseEvent p_evt) {
        ...
        Dimension panel_size = getSize();
        x = Math.min(x, panel_size.getWidth() - get_viewport_bounds().getWidth());
        y = Math.min(y, panel_size.getHeight() - get_viewport_bounds().getHeight());
    ...
    public void set_custom_crosshair_cursor(boolean p_value) {
        if (p_value) {
            custom_cursor = Cursor.get_45_degree_cross_hair_cursor();
        } else {
            custom_cursor = null;
        }
