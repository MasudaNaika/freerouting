...
/**
 * Class for transforming objects between user coordinate space and board
 * coordinate space.
 *
 * @author Alfons Wirtz
 */
public class CoordinateTransform implements Serializable {
    ...
    public PrintableShape board_to_user(geometry.planar.Shape p_shape, Locale p_locale) {
    ...
    public PrintableShape.Circle board_to_user(geometry.planar.Circle p_circle, Locale p_locale) {
        return new PrintableShape.Circle(board_to_user(p_circle.center.to_float()),
                board_to_user(p_circle.radius), p_locale);
    }

    public PrintableShape.Rectangle board_to_user(geometry.planar.IntBox p_box, Locale p_locale) {
        return new PrintableShape.Rectangle(board_to_user(p_box.ll.to_float()),
                board_to_user(p_box.ur.to_float()), p_locale);
    }

    public PrintableShape.Polygon board_to_user(geometry.planar.PolylineShape p_shape, Locale p_locale) {
