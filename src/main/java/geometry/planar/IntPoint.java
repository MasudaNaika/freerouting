...
public class IntPoint extends Point implements Serializable {
    ...
    public double signed_area(IntPoint p_1, IntPoint p_2) {
        ...
        IntVector d01 = (IntVector) difference_by(p_1);
    ...
    /**
     * Calculates the nearest point to this point on the horizontal or vertical
     * line through p_other (Snaps this point to on ortogonal line through
     * p_other).
     */
    public IntPoint orthogonal_projection(IntPoint p_other) {
        IntPoint result;
        int horizontal_distance = Math.abs(x - p_other.x);
        int vertical_distance = Math.abs(y - p_other.y);
        if (horizontal_distance <= vertical_distance) {
            // projection onto the vertical line through p_other
            result = new IntPoint(p_other.x, y);
        } else {
            // projection onto the horizontal line through p_other
            result = new IntPoint(x, p_other.y);
        }
        return result;
    }

    /**
     * Calculates the nearest point to this point on an orthogonal or diagonal
     * line through p_other (Snaps this point to on 45 degree line through
     * p_other).
     */
    ...
    /**
     * Calculates a corner point p so that the lines through this point and p
     * and from p to p_to_point are multiples of 45 degree, and that the angle
     * at p will be 45 degree. If p_left_turn, p_to_point will be on the left of
     * the line from this point to p, else on the right. Returns null, if the
     * line from this point to p_to_point is already a multiple of 45 degree.
     */
    public IntPoint fortyfive_degree_corner(IntPoint p_to_point, boolean p_left_turn) {
        int dx = p_to_point.x - x;
        int dy = p_to_point.y - y;
        IntPoint result;

        // handle the 8 sections between the 45 degree lines
        if (dy > 0 && dy < dx) {
            if (p_left_turn) {
                result = new IntPoint(p_to_point.x - dy, y);
            } else {
                result = new IntPoint(x + dy, p_to_point.y);
            }
        } else if (dx > 0 && dy > dx) {
            if (p_left_turn) {
                result = new IntPoint(p_to_point.x, y + dx);
            } else {
                result = new IntPoint(x, p_to_point.y - dx);
            }
        } else if (dx < 0 && dy > -dx) {
            if (p_left_turn) {
                result = new IntPoint(x, p_to_point.y + dx);
    ...
    /**
     * Calculates a corner point p so that the lines through this point and p
     * and from p to p_to_point are hprizontal or vertical, and that the angle
     * at p will be 90 degree. If p_left_turn, p_to_point will be on the left of
     * the line from this point to p, else on the right. Returns null, if the
     * line from this point to p_to_point is already orthogonal.
     */
    public IntPoint ninety_degree_corner(IntPoint p_to_point, boolean p_left_turn) {
        ...
        if (dx > 0 && dy > 0 || dx < 0 && dy < 0) {
            if (p_left_turn) {
                result = new IntPoint(p_to_point.x, y);
            } else {
                result = new IntPoint(x, p_to_point.y);
            }
