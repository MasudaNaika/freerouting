...
public class Circle implements ConvexShape, Serializable {
    ...
    public TileShape bounding_tile(int p_max_segment_length) {
        ...
        for (int i = 0; i < quadrant_division_count; ++i) {
            ...
            Vector border_delta;
            if (i == 0) {
                border_delta = new IntVector(radius, 0);
            } else {
            ...
            Point curr_a = center.translate_by(border_delta);
            Point curr_b = curr_a.turn_90_degree(1, center);
            Direction curr_dir = Direction.get_instance(curr_b.difference_by(center));
