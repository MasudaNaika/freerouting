...
public abstract class Item implements Drawable, SearchTreeObject, ObjectInfoPanel.Printable, UndoableObjects.Storable, Serializable {
    ...
    /**
     * Returns true if the net number array of this and p_net_no_arr have a
     * common number.
     */
    ...
    /**
     * Returns the number of shapes of this item after decomposition into convex
     * polygonal shapes
     */
    public abstract int tile_shape_count();

    /**
     * Returns the p_index-throws shape of this item after decomposition into
    ...
    public int tree_shape_count(ShapeTree p_tree) {
        ...
        TileShape[] precalculated_tree_shapes = get_precalculated_tree_shapes(p_tree);
    ...
    private TileShape[] get_precalculated_tree_shapes(ShapeTree p_tree) {
        if (search_trees_info == null) {
            search_trees_info = new ItemSearchTreesInfo();
        }
        TileShape[] precalculated_tree_shapes = search_trees_info.get_precalculated_tree_shapes(p_tree);
    ...
    /**
     * Creates a copy of this item with id number p_id_no. If p_id_no <= 0, the
     * id_no of the new item is generated internally
     */
    ...
    /**
     * Translates the shapes of this item by p_vector. Does not move the item in
     * the board.
     */
    public abstract void translate_by(Vector p_vector);

    /**
     * Turns this Item by p_factor times 90 degree around p_pole. Does not
     * update the item in the board.
     */
    public abstract void turn_90_degree(int p_factor, IntPoint p_pole);

    /**
     * Rotates this Item by p_angle_in_degree around p_pole. Does not update the
     * item in the board.
     */
    public abstract void rotate_approx(double p_angle_in_degree, FloatPoint p_pole);

    /**
     * Changes the placement side of this Item and mirrors it at the vertical
     * line through p_pole. Does not update the item in the board.
     */
    ...
    /**
     * Returns true, if some shapes of this item and p_other are on the same
     * layer.
     */
    ...
    public int clearance_violation_count() {
        Collection<ClearanceViolation> violations = clearance_violations();
        return violations.size();
    }

    /**
     * Returns a list of all clearance violations of this item with other items.
     * The objects in the list are of type ClearanceViolations. The first_item
     * in such an object is always this item.
     */
                ...
                if (is_obstacle) {
                    ...
                    if (!board.search_tree_manager.is_clearance_compensation_used()) {
                        double cl_offset = 0.5
                                * board.rules.clearance_matrix.value(curr_item.clearance_class, clearance_class, shape_layer(i));
    ...
    /**
     * Returns all connectable Items with a direct contacts to this item. The
    ...
    /**
     * Returns all connectable Items with a direct contacts to this item on the
    ...
    public boolean is_connected() {
        Collection<Item> contacts = get_all_contacts();
    ...
    public boolean is_connected_on_layer(int p_layer) {
        Collection<Item> contacts_on_layer = get_all_contacts(p_layer);
    ...
    /**
     * Returns the contact point, if this item and p_other are Connectable and
     * have a unique normal contact. Returns null otherwise
     */
    ...
    /**
     * Returns the set of all Connectable items of the net with number p_net_no
     * which can be reached recursively via normal contacts from this item. If
     * p_net_no <= 0, the net number is ignored.
     */
     ...
     * Returns the set of all Connectable items of the net with number p_net_no
     * which can be reached recursively via normal contacts from this item. If
     * p_net_no <= 0, the net number is ignored. If p_stop_at_plane, the
     * recursive algorithm stops, when a conduction area is reached, which does
     * not belong to a component.
     */
    ...
    /**
     * Recursive part of Trace.is_cycle. If p_ignore_areas is true, cycles where
     * conduction areas are involved are ignored.
     */
    ...
    /**
     * Returns the set of all Connectable items belonging to the net with number
     * p_net_no, which are not in the connected set of this item. If p_net_no <=
    ...
    /**
     * Returns all traces and vias from this item until the next fork or
     * terminal item.
     */
    ...
    /**
     * Returns all traces and vias from this item until the next fork or
     * terminal item. If p_stop_option == StopConnectionOption.FANOUT_VIA, the
     * algorithm will stop at the next fanout via, If p_stop_option ==
     * StopConnectionOption.VIA, the algorithm will stop at any via.
     */
    public Set<Item> get_connection_items(StopConnectionOption p_stop_option) {
        ...
        for (Item curr_item : contacts) {
            Point prev_contact_point = normal_contact_point(curr_item);
    ...
    /**
     * Returns all corners of this item, which are used for displaying the
     * ratsnest. To be overwritten in derived classes implementing the
     ...
     * p_id_no <= 0, it w2ill be generated internally.
     */
    @Override
    ...
    boolean is_delete_fixed() {
        ...
        if (component_no > 0 || is_user_fixed()) {
            return true;
        }
        // Also power planes are delete_fixed.
        if (this instanceof ConductionArea) {
            if (!board.layer_structure.arr[((ConductionArea) this).get_layer()].is_signal) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Returns true, if it is not allowed to change the location of this item by
     * the push algorithm.
     */
    public boolean is_shove_fixed() {
        return (fixed_state.ordinal() >= FixedState.SHOVE_FIXED.ordinal());
    }
    ...
    /**
     * Returns false, if this item is an obstacle for vias with the input net
     * number.
     */
    ...
    /**
     * gets the p_no-th net number of this item for 0 <= p_no <
     * this.net_count().
     */
    ...
    /**
     * Return the component number of this item or 0, if it does not belong to a
     * component.
     */
    ...
    /**
     * Removes p_net_no from the net number array. Returns false, if p_net_no
    ...
    /**
     * Sets the index in the clearance matrix describing the required spacing of
     * this item to other items.
     */
    ...
    public void change_clearance_class(int p_index) {
        ...
        clearance_class = p_index;
        clear_derived_data();
        if (board != null && board.search_tree_manager.is_clearance_compensation_used()) {
    ...
    /**
     * Makes this item connectable and assigns it to the input net. If p_net_no
     * < 0, the net items net number will be removed and the item will no longer
     * be connectable.
     */
    ...
    /**
     * Sets the item tree entries for the tree with identification number
     * p_tree_no.
     */
    @Override
    ...
    /**
     * Returns the tree entries for the tree with identification number
     * p_tree_no, or null, if for this tree no entries of this item are
     * inserted.
     */
    ...
    /**
     * Sets the precalculated tree shapes tree entries for the tree with
     * identification number p_tree_no.
     */
    ...
    public void clear_search_tree_entries() {
        search_trees_info = null;
    }

    /**
     * Gets the information for the autoroute algorithm. Creates it, if it does
     * not yet exist.
     */
    ...
    protected void print_net_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        for (int i = 0; i < net_count(); ++i) {
            ...
            rules.Net curr_net = board.rules.nets.get(get_net_no(i));
    ...
    protected void print_clearance_info(ObjectInfoPanel p_window, Locale p_locale) {
        if (clearance_class > 0) {
            ...
            String name = board.rules.clearance_matrix.get_name(clearance_class);
            p_window.append(name, resources.getString("clearance_info"), board.rules.clearance_matrix.get_row(clearance_class));
        }
    }
    ...
    protected void print_fixed_info(ObjectInfoPanel p_window, Locale p_locale) {
        if (fixed_state != FixedState.UNFIXED) {
            ResourceBundle resources
                    = ResourceBundle.getBundle("board.resources.FixedState", p_locale);
            p_window.append(", ");
    ...
    protected void print_contact_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        if (!contacts.isEmpty()) {
            ...
            p_window.append_items(Integer.toString(contact_count), resources.getString("contact_info"), contacts);
        }
    }
    ...
    protected void print_clearance_violation_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        if (!clearance_violations.isEmpty()) {
            ...
            Collection<ObjectInfoPanel.Printable> violations = new LinkedList<>();
            violations.addAll(clearance_violations);
            p_window.append_objects(Integer.toString(violation_count), resources.getString("violation_info"), violations);
    ...
    public boolean nets_equal(int[] p_net_no_arr) {
        if (net_no_arr.length != p_net_no_arr.length) {
            return false;
        }
        for (int curr_net_no : p_net_no_arr) {
            if (!contains_net(curr_net_no)) {
                return false;
            }
        }
        return true;
    ...
    /**
     * Returns true, if the via is directly ob by a trace connected to a nearby
     * SMD-pin. If p_ignore_items != null, contact traces in P-ignore_items are
     * ignored.
     */
