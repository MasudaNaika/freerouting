...
public abstract class Trace extends Item implements Connectable, Serializable {
    ...
    /**
     * Returns the half with enlarged by the clearance compensation value for
     * the tree with id number p_ttree_id_no Equals get_half_width(), if no
     * clearance compensation is used in this tree.
     */
    public int get_compensated_half_width(ShapeSearchTree p_search_tree) {
        int result = half_width + p_search_tree.clearance_compensation_value(clearance_class_no(), layer);
        return result;
    }
    ...
    public boolean is_tail() {
        Collection<Item> contact_list = get_start_contacts();
    ...
    /**
     * Get a list of all items having a connection point at p_point on the layer
     * of this trace. If p_ignore_net is false, only contacts to items sharing a
    ...
    Point normal_contact_point(Trace p_other) {
        if (layer != p_other.layer) {
            return null;
        }
        boolean contact_at_first_corner
                = first_corner().equals(p_other.first_corner())
                || first_corner().equals(p_other.last_corner());
        boolean contact_at_last_corner
                = last_corner().equals(p_other.first_corner())
                || last_corner().equals(p_other.last_corner());
        Point result;
    ...
    /**
     * looks, if this trace is connectet to the same object at its start and its
     * end point
     */
    @Override
    public boolean is_overlap() {
        Set<Item> start_contacts = get_start_contacts();
        Set<Item> end_contacts = get_end_contacts();
        for (Item item : end_contacts) {
            if (start_contacts.contains(item)) {
                return true;
            }
        }
        return false;
    ...
    public boolean is_shove_fixed() {
        ...
        rules.Nets nets = board.rules.nets;
    ...
    /**
     * returns the endpoint of this trace with the shortest distance to
     * p_from_point
     */
    ...
    /**
     * Checks, if this trace can be reached by other items via more than one
     * path
     */
    ...
    /**
     * Looks up touching pins at the first corner and the last corner of the
    ...
    Set<Pin> touching_pins_at_end_corners() {
        ...
        Point curr_end_point = first_corner();
        for (int i = 0; i < 2; ++i) {
            IntOctagon curr_oct = curr_end_point.surrounding_octagon();
            curr_oct = curr_oct.enlarge(half_width);
            Set<Item> curr_overlaps = board.overlapping_items_with_clearance(curr_oct, layer, new int[0], clearance_class_no());
    ...
    public void print_info(ObjectInfoPanel p_window, Locale p_locale) {
        ...
        p_window.append(board.layer_structure.arr[layer].name);
        p_window.append(", " + resources.getString("width") + " ");
        p_window.append(2 * half_width);
     ...
     * from splitting. If nothing is split, the result will contain just this
     * Trace. If p_clip_shape != null, the split may be resticted to
     * p_clip_shape.
     */
