...
public class MoveItemState extends InteractiveState {
    ...
    public static MoveItemState get_instance(FloatPoint p_location, Collection<Item> p_item_list,
        ...
        ResourceBundle resources = ResourceBundle.getBundle("interactive.resources.InteractiveState", p_board_handling.get_locale());
    ...
    private MoveItemState(FloatPoint p_location, Set<Item> p_item_list, Set<Component> p_component_list,
        ...
        observers_activated = !hdlg.get_routing_board().observers_active();
    ...
    private void add_to_net_items_list(Item p_item, int p_net_no) {
        for (NetItems curr_items : net_items_list) {
    ...
    public InteractiveState mouse_moved() {
        ...
        if (logfile != null) {
            logfile.add_corner(current_position.to_float());
        }
        return this;
    }
    ...
    public InteractiveState complete() {
        ...
        for (Component curr_component : component_list) {
            routing_board.communication.observers.notify_moved(curr_component);
        }

        for (NetItems curr_net_items : net_items_list) {
            hdlg.update_ratsnest(curr_net_items.net_no);
        }
    ...
    private void move(FloatPoint p_new_position) {
        ...
        if (!current_position.equals(previous_position)) {
            ...
            clearance_violations = new LinkedList<>();
            for (Item curr_item : item_list) {
            ...
            previous_position = current_position;
            for (NetItems curr_net_items : net_items_list) {
                hdlg.update_ratsnest(curr_net_items.net_no, curr_net_items.items);
            }
            hdlg.repaint();
        }
    }
    ...
    private Vector adjust_to_placement_grid(Vector p_vector) {
        Point new_component_location = grid_snap_component.get_location().translate_by(p_vector);
    ...
    /**
     * Turns the items in the list by p_factor times 90 degree around the
     * current position.
     */
    ...
    public void rotate(double p_angle_in_degree) {
        ...
        board.Components components = hdlg.get_routing_board().components;
        for (Component curr_component : component_list) {
            components.rotate(curr_component.no, p_angle_in_degree, current_position);
        }
        clearance_violations = new LinkedList<>();
        FloatPoint float_position = current_position.to_float();
        for (Item curr_item : item_list) {
