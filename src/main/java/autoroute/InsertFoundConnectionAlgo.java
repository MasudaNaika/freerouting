...
public class InsertFoundConnectionAlgo {
    ...
    /**
     * Creates a new instance of InsertFoundConnectionAlgo . Returns null, if
    ...
    /**
     * Inserts the trace by shoving aside obstacle traces and vias. Returns
     * false, that was not possible for the whole trace.
     */
    private boolean insert_trace(LocateFoundConnectionAlgoAnyAngle.ResultItem p_trace) {
        ...
        if (ctrl.with_neckdown) {
            ...
            for (int i = 0; i < 2; ++i) {
                Set<Item> picked_items = board.pick_items(curr_end_corner, p_trace.layer, item_filter);
    ...
    private Point try_neck_down(Point p_from_corner, Point p_to_corner, int p_layer, board.Pin p_pin, boolean p_at_start) {
        ...
        double curr_clearance
                = board.rules.clearance_matrix.value(ctrl.trace_clearance_class_no, p_pin.clearance_class_no(), p_layer);
    ...
    /**
     * Searchs the cheapest via masks containing p_from_layer and p_to_layer, so
     * that a forced via is possible at p_location with this mask and inserts
     * the via. Returns false, if no suitable via mmask was found or if the
     * algorithm failed.
     */
