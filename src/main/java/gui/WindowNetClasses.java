...
public class WindowNetClasses extends BoardSavableSubWindow {
    ...
    public WindowNetClasses(BoardFrame p_board_frame) {
        resources = ResourceBundle.getBundle("gui.resources.WindowNetClasses", p_board_frame.get_locale());
        ...
        via_rule_combo_box = new JComboBox();
        ...
        main_panel.add(net_class_button_panel, BorderLayout.SOUTH);
        final JButton add_class_button = new JButton(resources.getString("add"));
        ...
        final JButton remove_class_button = new JButton(resources.getString("remove"));
        ...
        final JButton assign_button = new JButton(resources.getString("assign"));
        ...
        final JButton select_button = new JButton(resources.getString("select"));
        ...
        final JButton contained_nets_button = new JButton(resources.getString("show_nets"));
    ...
    private void add_table() {
        table_model = new NetClassTableModel();
        table = new NetClassTable(table_model);
        ...
        if (board_frame.get_locale().getLanguage().equalsIgnoreCase("de")) {
            JLabel bug_label
                    = new JLabel("Wegen eines Java-System-Bugs muss das Dezimalkomma in dieser Tabelle zur Zeit als Punkt eingegeben werden!");
            center_panel.add(bug_label, BorderLayout.SOUTH);
        }
        main_panel.add(center_panel, BorderLayout.CENTER);

        table.getColumnModel().getColumn(ColumnName.CLEARANCE_CLASS.ordinal()).setCellEditor(new DefaultCellEditor(cl_class_combo_box));

        table.getColumnModel().getColumn(ColumnName.VIA_RULE.ordinal()).setCellEditor(new DefaultCellEditor(via_rule_combo_box));
        table.getColumnModel().getColumn(ColumnName.ON_LAYER.ordinal()).setCellEditor(new DefaultCellEditor(layer_combo_box));
    }
    ...
    private JComboBox cl_class_combo_box;
    private JComboBox via_rule_combo_box;
    ...
    private class NetClassTableModel extends AbstractTableModel {
        ...
        public void set_values() {
            ...
            data = new Object[board_rules.net_classes.count()][];
            for (int i = 0; i < data.length; ++i) {
                data[i] = new Object[ColumnName.values().length];
            }
            for (int i = 0; i < data.length; ++i) {
                NetClass curr_net_class = board_rules.net_classes.get(i);
                data[i][ColumnName.NAME.ordinal()] = curr_net_class.get_name();
                if (curr_net_class.get_via_rule() != null) {
                    data[i][ColumnName.VIA_RULE.ordinal()] = curr_net_class.get_via_rule().name;
                }
                data[i][ColumnName.SHOVE_FIXED.ordinal()] = curr_net_class.is_shove_fixed() || !curr_net_class.get_pull_tight();
                data[i][ColumnName.CYCLES_WITH_AREAS.ordinal()] = curr_net_class.get_ignore_cycles_with_areas();
                ...
                data[i][ColumnName.MIN_TRACE_LENGTH.ordinal()] = (float) min_trace_length;
                ...
                data[i][ColumnName.MAX_TRACE_LENGTH.ordinal()] = (float) max_trace_length;
                data[i][ColumnName.CLEARANCE_CLASS.ordinal()]
                ...
                data[i][ColumnName.ON_LAYER.ordinal()] = combo_layer.name;
            }
        }
        ...
        void set_trace_width_field(int p_rule_no, ComboBoxLayer.Layer p_layer) {
            ...
            data[p_rule_no][ColumnName.TRACE_WIDTH.ordinal()] = trace_width;
