...
public class Components implements Serializable {
     ...
     * inserted seperately into the board. If p_on_front is false, the component
     * will be placed on the back side, and p_package_back is used instead of
     * p_package_front.
     */
    ...
    /**
     * Adds a component to this object. The items of the component have to be
     * inserted seperately into the board. If p_on_front is false, the component
     * will be placed on the back side. The component name is generated
     * internally.
     */
    ...
    /**
     * Returns the component with the input name or null, if no such component
     * exists.
     */
    ...
    /**
     * Returns the component with the input component number or null, if no such
    ...
    /**
     * Restores the sitiation at the previous snapshot. Returns false, if no
     * more undo is possible.
     */
    public boolean undo(BoardObservers p_observers) {
        if (!undo_list.undo(null, null)) {
            return false;
        }
    ...
    /**
     * Restores the sitiation before the last undo. Returns false, if no more
     * redo is possible.
     */
    public boolean redo(BoardObservers p_observers) {
        if (!undo_list.redo(null, null)) {
            return false;
        }
        restore_component_arr_from_undo_list(p_observers);
        return true;
    ...
    private void restore_component_arr_from_undo_list(BoardObservers p_observers) {
        Iterator<UndoableObjects.UndoableObjectNode> it = undo_list.start_read_object();
        while (true) {
            Component curr_component = (Component) undo_list.read_object(it);
            if (curr_component == null) {
                break;
            }
            component_arr.set(curr_component.no - 1, curr_component);
            p_observers.notify_moved(curr_component);
        }
    }

    /**
     * Moves the component with number p_component_no. Works contrary to
     * Component.translate_by with the undo algorithm of the board.
     */
    public void move(int p_component_no, geometry.planar.Vector p_vector) {
        Component curr_component = get(p_component_no);
        undo_list.save_for_undo(curr_component);
        curr_component.translate_by(p_vector);
    }

    /**
     * Turns the component with number p_component_no by p_factor times 90
     * degree around p_pole. Works contrary to Component.turn_90_degree with the
    ...
    /**
     * Rotates the component with number p_component_no by p_rotation_in_degree
     * around p_pole. Works contrary to Component.rotate with the undo algorithm
     * of the board.
     */
    ...
    /**
     * Changes the placement side of the component the component with
     * numberp_component_no and mirrors it at the vertical line through p_pole.
    ...
    private List<Component> component_arr = new ArrayList<>();
