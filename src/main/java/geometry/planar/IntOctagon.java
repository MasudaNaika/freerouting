...
public class IntOctagon extends RegularTileShape implements Serializable {
     ...
     * of the shape. p_ly is the smallest y value of the shape. p_rx is the
     * biggest x valuje af the shape. p_uy is the biggest y value of the shape.
     * p_ulx is the intersection of the upper left diagonal boundary line with
     * the x axis. p_lrx is the intersection of the lower right diagonal
     * boundary line with the x axis. p_llx is the intersection of the lower
     * left diagonal boundary line with the x axis. p_urx is the intersection of
     * the upper right diagonal boundary line with the x axis.
     */
    ...
    /**
     * Additional to the function corner() for performance reasons to avoid
     * allocation of an IntPoint.
     */
    ...
    public IntOctagon normalize() {
        ...
        if (new_lx < new_llx - new_uy) // the point new_lx, new_uy is the the lower left border line of
        // this octagon
        ...
        if (new_lx < new_ulx + new_ly) // the point new_lx, new_ly is above the the upper left border line of
        // this octagon
        ...
        if (new_rx > new_urx - new_ly) // the point new_rx, new_ly is above the the upper right border line of
        // this octagon
        ...
        if (new_rx > new_lrx + new_uy) // the point new_rx, new_uy is below the the lower right border line of
        // this octagon
        ...
        if (new_ly < new_lx - new_lrx) // the point lx, ly is below the lower right border line of this
        // octagon
        ...
        if (new_ly < new_llx - new_rx) // the point rx, ly is below the lower left border line of
        // this octagon.
        ...
        if (new_uy > new_urx - new_lx) // the point lx, uy is above the upper right border line of
        // this octagon.
        ...
        if (new_uy > new_rx - new_ulx) // the point rx, uy is above the upper left border line of
        // this octagon.
        ...
        if (new_llx - new_lx < new_ly) // The point lx, ly is above the lower left border line of
        // this octagon.
        ...
        if (new_rx - new_lrx < new_ly) // the point rx, ly is above the lower right border line of
        // this octagon.
        ...
        if (new_urx - new_rx > new_uy) // the point rx, uy is below the upper right border line of p_oct.
        // Change the upper right line, so that it runs through this point.
        {
            new_urx = new_uy + new_rx;
        }

        if (new_lx - new_ulx > new_uy) // the point lx, uy is below the upper left border line of
        // this octagon.
        ...
        if (new_uy > diag_upper_y) // the intersection of the upper right and the upper left border
        ...
        if (new_ly < diag_lower_y) // the intersection of the lower right and the lower left border
        ...
        if (new_rx > diag_right_x) // the intersection of the upper right and the lower right border
        ...
        if (new_lx < diag_left_x) // the intersection of the lower left and the upper left border
    ...
    /**
     * Returns true, if p_point is contained in this octagon. Because of the
     * parameter type FloatPoint, the function may not be exact close to the
     * border.
     */
    @Override
    ...
    /**
     * Calculates the side of the point (p_x, p_y) of the border line with index
     * p_border_line_no. The border lines are located in counterclock sense
     * around this octagon.
     */
    ...
    /**
     * Returns true, if this octagon intersects with p_other and the
     * intersection is 2-dimensional.
     */
    ...
    /**
     * Calculates the border point of this octagon from p_point into the 45
     * degree direction p_dir. If this border point is not an IntPoint, the
     * nearest outside IntPoint of the octagon is returned.
     */
    ...
    /**
     * Calculates the sorted p_max_result_points nearest points on the border of
     * this octagon in the 45-degree directions. p_point is assumed to be
     * located in the interiour of this octagon.
     */
    public IntPoint[] nearest_border_projections(IntPoint p_point, int p_max_result_points) {
        if (!contains(p_point) || p_max_result_points <= 0) {
    ...
    Side border_line_side_of(FloatPoint p_point, int p_line_no, double p_tolerance) {
        ...
        switch (p_line_no) {
            case 0:
                if (p_point.y > ly + p_tolerance) {
                    result = Side.ON_THE_RIGHT;
                } else if (p_point.y < ly - p_tolerance) {
                    result = Side.ON_THE_LEFT;
                } else {
                    result = Side.COLLINEAR;
                }
                break;
            case 2:
                if (p_point.x < rx - p_tolerance) {
                    result = Side.ON_THE_RIGHT;
                } else if (p_point.x > rx + p_tolerance) {
                    result = Side.ON_THE_LEFT;
                } else {
                    result = Side.COLLINEAR;
                }
                break;
            case 4:
                if (p_point.y < uy - p_tolerance) {
                    result = Side.ON_THE_RIGHT;
                } else if (p_point.y > uy + p_tolerance) {
                    result = Side.ON_THE_LEFT;
                } else {
                    result = Side.COLLINEAR;
                }
                break;
            case 6:
                if (p_point.x > lx + p_tolerance) {
                    result = Side.ON_THE_RIGHT;
                } else if (p_point.x < lx - p_tolerance) {
                    result = Side.ON_THE_LEFT;
                } else {
                    result = Side.COLLINEAR;
                }
    ...
    /**
     * Divide p_d minus this octagon into 8 convex pieces, from which 4 have cut
     * off a corner.
     */
    @Override
    IntOctagon[] cutout_from(IntBox p_d) {
        IntOctagon c = intersection(p_d);

        if (is_empty() || c.dimension() < dimension()) {
            // there is only an overlap at the border
            IntOctagon[] result = {p_d.to_IntOctagon()};
            return result;
        }

        IntBox[] boxes = {
            // construct left box
            new IntBox(p_d.ll.x, c.llx - c.lx, c.lx, c.lx - c.ulx),
            // construct right box
            new IntBox(c.rx, c.rx - c.lrx, p_d.ur.x, c.urx - c.rx),
            // construct lower box
            new IntBox(c.llx - c.ly, p_d.ll.y, c.lrx + c.ly, c.ly),
            // construct upper box
            new IntBox(c.ulx + c.uy, c.uy, c.urx - c.uy, p_d.ur.y)};
    ...
    /**
     * Divide p_divide_octagon minus cut_octagon into 8 convex pieces without
     * sharp angles.
     */
    @Override
    IntOctagon[] cutout_from(IntOctagon p_d) {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                && curr_2.upper_y_value(curr_1.rx) - curr_2.ly
                > curr_1.rx - curr_1.left_x_value(curr_2.ly)) // switch the vertical upper left divide line to horizontal
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_2.right_x_value(curr_1.ly) - curr_2.lx) // switch the vertical upper right divide line to horizontal
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_1.upper_y_value(curr_2.lx) - curr_2.uy) // switch the horizontal upper right divide line to vertical
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_1.ly - curr_2.lower_y_value(curr_1.lx)) // switch the horizontal lower right divide line to vertical
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_1.right_x_value(curr_2.uy) - curr_2.rx) // switch the vertical lower right divide line to horizontal
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_1.lx - curr_2.left_x_value(curr_1.uy)) // switch the vertical lower left divide line to horizontal
        {
        ...
        if (!(curr_1.is_empty() || curr_2.is_empty())
                ...
                > curr_2.ly - curr_1.lower_y_value(curr_2.rx)) // switch the horizontal lower left divide line to vertical
        {
