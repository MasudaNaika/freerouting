...
public class Route {
    ...
    public boolean next_corner(FloatPoint p_corner) {
        ...
        if (!(board.contains(prev_corner) && board.contains(curr_corner) && board.layer_structure.arr[layer].is_signal)) {
            return false;
        }
        ...
        if (nearest_target_item instanceof DrillItem) {
            ...
            if (prev_corner.equals(target.get_center())) {
        ...
        Item end_routing_item = board.pick_nearest_routing_item(prev_corner, layer, null);
        // look for a nearby item of this net, which is not connected to end_routing_item.
        nearest_target_item = board.pick_nearest_routing_item(curr_corner, layer, end_routing_item);
        TimeLimit check_forced_trace_time_limit;
        if (is_stitch_mode || board.get_test_level() != TestLevel.RELEASE_VERSION) {
        ...
        if (ok_point == prev_corner && with_neckdown) {
    ...
    public boolean change_layer(int p_to_layer) {
        ...
        if (via_snap_to_smd_center) {
            ...
            if (!snapped_to_smd_center) {
                snap_to_smd_center(layer);
            }
        }
        boolean result = true;
        int min_layer = Math.min(layer, p_to_layer);
        int max_layer = Math.max(layer, p_to_layer);
        boolean via_found = false;
        for (int i = 0; i < via_rule.via_count(); ++i) {
            ViaInfo curr_via_info = via_rule.get_via(i);
            ...
            result = board.forced_via(curr_via_info, prev_corner, net_no_arr, clearance_class,
                    pen_half_width_arr, max_shove_trace_recursion_depth,
                    0, trace_tidy_width, pull_tight_accuracy, pull_tight_time_limit);
            if (result) {
                via_found = true;
                break;
            }
    ...
    private boolean snap_to_smd_center(int p_layer) {
        ...
        Collection<Item> picked_items = board.pick_items(prev_corner, p_layer, selection_filter);
        board.Pin found_smd_pin = null;
        for (Item curr_item : picked_items) {
            if (curr_item instanceof board.Pin && curr_item.shares_net_no(net_no_arr)) {
    ...
    private boolean connect_to_target(IntPoint p_from_point) {
        ...
        if (nearest_target_item instanceof DrillItem) {
            ...
            return board.connect_to_trace(p_from_point, (PolylineTrace) nearest_target_item,
                    pen_half_width_arr[layer], clearance_class);
        ...
        for (int i = 1; i < corners.length; ++i) {
            ...
            while (!from_corner.equals(to_corner)) {
                Point curr_ok_point = board.insert_forced_trace_segment(from_corner,
                        to_corner, pen_half_width_arr[layer], layer, net_no_arr,
                ...
                if (curr_ok_point.equals(from_corner) && with_neckdown) {
    ...
    private Set<SwapPinInfo> calculate_swap_pin_infos() {
        ...
        ItemSelectionFilter selection_filter = new ItemSelectionFilter(ItemSelectionFilter.SelectableChoices.PINS);
        Collection<Item> picked_items = board.pick_items(prev_corner, layer, selection_filter);
        for (Item curr_item : picked_items) {
            if (curr_item instanceof board.Pin) {
                Collection<board.Pin> curr_swapppable_pins = ((board.Pin) curr_item).get_swappable_pins();
                for (board.Pin curr_swappable_pin : curr_swapppable_pins) {
    ...
    public void draw(Graphics p_graphics, GraphicsContext p_graphics_context) {
        if (hilight_shove_failing_obstacle && shove_failing_obstacle != null) {
        ...
        for (SwapPinInfo curr_info : swap_pin_infos) {
        ...
        FloatPoint from_corner = prev_corner.to_float();
        if (nearest_target_point != null && prev_corner != null) {
            ...
            if (nearest_target_item != null && !nearest_target_item.is_on_layer(layer)) {
    ...
    private void set_shove_failing_obstacle(Item p_item) {
        ...
        if (p_item != null) {
            board.join_graphics_update_box(p_item.bounding_box());
        }
    }
    ...
    private Point try_neckdown_at_start(IntPoint p_to_corner) {
        ...
        board.Pin start_pin = (board.Pin) start_item;
        if (!start_pin.is_on_layer(layer)) {
            return prev_corner;
        }
        FloatPoint pin_center = start_pin.get_center().to_float();
        double curr_clearance
                = board.rules.clearance_matrix.value(clearance_class, start_pin.clearance_class_no(), layer);
        double pin_neck_down_distance
                = 2 * (0.5 * start_pin.get_max_width(layer) + curr_clearance);
        if (pin_center.distance(prev_corner.to_float()) >= pin_neck_down_distance) {
            return prev_corner;
        }

        int neck_down_halfwidth = start_pin.get_trace_neckdown_halfwidth(layer);
        if (neck_down_halfwidth >= pen_half_width_arr[layer]) {
            return prev_corner;
        }

        // check, that the neck_down started inside the pin shape
        if (!prev_corner.equals(start_pin.get_center())) {
            Item picked_item = board.pick_nearest_routing_item(prev_corner, layer, null);
     ...
     * width, another try with the smalllest pin width is done. Returns the
     * ok_point of the try, which is p_from_corner, if the try failed.
     */
    private Point try_neckdown_at_end(Point p_from_corner, Point p_to_corner) {
        if (!(nearest_target_item instanceof board.Pin)) {
            return p_from_corner;
        }
        board.Pin target_pin = (board.Pin) nearest_target_item;
        if (!target_pin.is_on_layer(layer)) {
            return p_from_corner;
        }
        FloatPoint pin_center = target_pin.get_center().to_float();
        double curr_clearance
                = board.rules.clearance_matrix.value(clearance_class, target_pin.clearance_class_no(), layer);
        double pin_neck_down_distance
                = 2 * (0.5 * target_pin.get_max_width(layer) + curr_clearance);
        if (pin_center.distance(p_from_corner.to_float()) >= pin_neck_down_distance) {
            return p_from_corner;
        }
        int neck_down_halfwidth = target_pin.get_trace_neckdown_halfwidth(layer);
        if (neck_down_halfwidth >= pen_half_width_arr[layer]) {
            return p_from_corner;
        }
        TimeLimit time_limit = new TimeLimit(CHECK_FORCED_TRACE_TIME_LIMIT);
    ...
    private class SwapPinInfo implements Comparable<SwapPinInfo> {
        ...
        SwapPinInfo(board.Pin p_pin) {
            ...
            for (Item curr_item : net_items) {
                if (curr_item == pin || !(curr_item instanceof DrillItem)) {
                    continue;
                }
        ...
        public int compareTo(SwapPinInfo p_other) {
            return pin.compareTo(p_other.pin);
        }
        final board.Pin pin;
