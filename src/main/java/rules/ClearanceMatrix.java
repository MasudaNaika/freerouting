...
public class ClearanceMatrix implements Serializable {
    ...
    public ClearanceMatrix(int p_class_count, board.LayerStructure p_layer_structure, String[] p_name_arr) {
        ...
        max_value_on_layer = new int[layer_structure.arr.length];
    }

    /**
     * Returns the number of the clearance class with the input name, or -1, if
     * no such clearance class exists.
     */
    ...
    /**
     * Sets the value of all clearance classes with number >= 1 to p_value on
     * all layers.
     */
    ...
    /**
     * Sets the value of an entry in the clearance matrix to p_value on all
     * layers.
     */
    ...
    /**
     * Cets the required spacing of clearance classes with index p_i and p_j on
     * p_layer. This value will be always an even integer.
     */
    ...
    /**
     * Returns the maximal required spacing of clearance class with index p_i to
     * all other clearance classes on layer p_layer.
     */
    ...
    /**
     * Returns true, if the values of the clearance matrix in the p_i-th column
     * and the p_j-th row are not equal on all layers.
     */
    ...
    /**
     * Return the clearance compensation value of p_clearance_class_no on layer
     * p_layer.
     */
    public int clearance_compensation_value(int p_clearance_class_no, int p_layer) {
        return (value(p_clearance_class_no, p_clearance_class_no, p_layer) + 1) / 2;
    }

    /**
     * Appends a new clearance class to the clearence matrix and initializes it
     * with the values of the default class. Returns false, oif a clearance
     * class with name p_class_name is already existing.
     */
    ...
    /**
     * Returns true, if all clearance values of the class with index p_1 are
     * equal to the clearance values of index p_2.
     */
     ...
     */
    private class Row implements board.ObjectInfoPanel.Printable, Serializable {
        ...
        public void print_info(board.ObjectInfoPanel p_window, Locale p_locale) {
            ...
            for (int i = 1; i < column.length; ++i) {
                ...
                MatrixEntry curr_column = column[i];
