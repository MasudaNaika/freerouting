...
/**
 * Contains the rules and constraints required for items to be inserted into a
 * routing board
 *
 * @author Alfons Wirtz
 */
public class BoardRules implements Serializable {
    ...
    public BoardRules(board.LayerStructure p_layer_structure, ClearanceMatrix p_clearance_matrix) {
        ...
        nets = new Nets();
        trace_angle_restriction = board.AngleRestriction.FORTYFIVE_DEGREE;
    ...
    /**
     * Returns the trace halfwidth used for routing with the input net on the
     * input layer.
     */
     ...
     * equal on all layers. If p_net_no < 0, the default trace widths for all
     * nets are checked.
     */
    ...
    /**
     * Changes the default trace halfwidth used for routing on all layers to the
     * input value.
     */
    ...
    /**
     * Returns the net rule used for all nets, for whichh no special rrule was
     * set.
     */
    ...
    /**
     * Create a default via rule for p_net_class with name p_name. If more than
     * one via infos with the same layer range are found, only the via info with
     * the smmallest pad size is inserted.
     */
    public void create_default_via_rule(NetClass p_net_class, String p_name) {
        ...
        for (int i = 0; i < via_infos.count(); ++i) {
            ViaInfo curr_via_info = via_infos.get(i);
    ...
    /**
     * Appends a new net class initialized with default data and returns that
     * class. If a class with p_name exists, this class is returned withoout
     * appending a new class.
     */
    ...
    /**
     * Changes the clearance class index of all objects on the board with index
     * p_from_no to p_to_no.
     */
    public void change_clearance_class_no(int p_from_no, int p_to_no, Collection<board.Item> p_board_items) {
        ...
        for (int i = 0; i < via_infos.count(); ++i) {
            rules.ViaInfo curr_via = via_infos.get(i);
    ...
    /**
     * Removes the clearance class with number p_index. Returns false, if that
     * was not possible, because there were still items assigned to this class.
     */
    public boolean remove_clearance_class(int p_index, Collection<board.Item> p_board_items) {
        for (board.Item curr_item : p_board_items) {
     ...
     * of a connected trace por a pin with connection restrictions. If the
     * result is <= 0, there are no exit restrictions.
     */
    ...
    /**
     * Sets he minimum distance between the pin border and the next corner of a
     * connected trace por a pin with connection restrictions. if p_value is <=
     * 0, there are no exit restrictions.
     */
    ...
    /**
     * If true, shapes of type Simplex are always used in the autorouter
     * algorithm. If false, shapes of type IntBox are used in 90 degree
     * autorouting and shapes of type IntOctagon are used in 45 degree
     * autorouting.
     */
    ...
    /**
     * Returns the Maximum of the diameter of the default via on its first and
     * last layer.
     */
    public double get_default_via_diameter() {
        ViaRule default_via_rule = get_default_via_rule();
     ...
     */
    private void writeObject(ObjectOutputStream p_stream)
            throws IOException {
    ...
    private void readObject(ObjectInputStream p_stream)
        ...
        trace_angle_restriction = board.AngleRestriction.arr[snap_angle_no];
    }
     ...
     * trace to a pin with restricted exit directions. If the value is <= 0,
