...
/**
 *
 * Implements functionality of orthogonal rectangles in the plane with integer
 * coordinates.
 *
 *
 * @author Alfons Wirtz
...
public class IntBox extends RegularTileShape implements Serializable {
    ...
    /**
     * creates an IntBox from the coordinates of its lower left and upper right
     * corners.
     */
    ...
    public boolean contains_inside(IntPoint p_point) {
        return p_point.x > ll.x && p_point.x < ur.x
                && p_point.y > ll.y && p_point.y < ur.y;
    }
     ...
     * this box. p_point is assumed to be located in the interiour of this nox.
    ...
    public double weighted_distance(IntBox p_other, double p_horizontal_weight, double p_vertical_weight) {
        ...
        double max_ll_x = Math.max(ll.x, p_other.ll.x);
        double max_ll_y = Math.max(ll.y, p_other.ll.y);
        double min_ur_x = Math.min(ur.x, p_other.ur.x);
        double min_ur_y = Math.min(ur.y, p_other.ur.y);
    ...
    /**
     * Returns true, if this box intersects with p_other and the intersection is
     * 2-dimensional.
     */
    ...
    /**
     * Enlarges the box by p_offset. Contrary to the offset() method the result
     * is an IntOctagon, not an IntBox.
     */
    @Override
    ...
    /**
     * Returns the box offseted by p_dist. If p_dist > 0, the offset is to the
     * outside, else to the inside.
     */
    @Override
     ...
     * p_dist > 0, the offset is to the outside, else to the inside.
     */
    ...
    public IntBox nearest_part(IntBox p_from_box) {
        ...
        if (p_from_box.ll.x >= ll.x) {
            ll_x = p_from_box.ll.x;
        } else if (p_from_box.ur.x >= ll.x) {
            ll_x = ll.x;
        } else {
        ...
        if (p_from_box.ur.x <= ur.x) {
            ur_x = p_from_box.ur.x;
        } else if (p_from_box.ll.x <= ur.x) {
            ur_x = ur.x;
        } else {
        ...
        if (p_from_box.ll.y >= ll.y) {
            ll_y = p_from_box.ll.y;
        } else if (p_from_box.ur.y >= ll.y) {
            ll_y = ll.y;
        } else {
        ...
        if (p_from_box.ur.y <= ur.y) {
            ur_y = p_from_box.ur.y;
        } else if (p_from_box.ll.y <= ur.y) {
            ur_y = ur.y;
        } else {
    ...
    public IntBox[] divide_into_sections(double p_max_section_width) {
        ...
        for (int j = 0; j < y_count; ++j) {
            int curr_lly = ll.y + j * section_length_y;
            int curr_ury;
            if (j == (y_count - 1)) {
                curr_ury = ur.y;
            } else {
            ...
            for (int i = 0; i < x_count; ++i) {
                int curr_llx = ll.x + i * section_length_x;
                int curr_urx;
                if (i == (x_count - 1)) {
                    curr_urx = ur.x;
                } else {
