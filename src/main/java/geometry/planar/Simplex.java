...
/**
 *
 * Convex shape defined as intersection of half-planes. A half-plane is defined
 * as the positive side of a directed line.
 *
 * @author Alfons Wirtz
 */
public class Simplex extends TileShape implements Serializable {
    ...
    /**
     * Returns true, if the determinant of the direction of index p_no -1 and
     * the direction of index p_no is > 0
     */
    @Override
    ...
    /**
     * Returns true, if the shape of this simplex is contained in a sufficiently
     * large box
     */
    @Override
    ...
    /**
     * Returns the intersection of the p_no -1-th with the p_no-th line of this
     * simplex. If the simplex is not bounded at this corner, the coordinates of
     ...
     * p_no-th line of this simplex by a FloatPoint. If the simplex is not
     * bounded at this corner, the coordinates of the result will be set to
     * Integer.MAX_VALUE.
     */
    @Override
    ...
    public FloatPoint[] corner_approx_arr() {
        ...
        for (int i = 0; i < precalculated_float_corners.length; ++i) {
            if (precalculated_float_corners[i] == null) // corner is not yet calculated
            {
                Line prev;
                if (i == 0) {
                    prev = arr[arr.length - 1];
    ...
    /**
     * returns the p_no-th edge line of this simplex. The edge lines are sorted
    ...
    /**
     * Returns the dimension of this simplex. The result may be 2, 1, 0, or -1
    ...
    /**
     * Converts this IntSimplex to an IntOctagon. Returns null, if that is not
     * possible, because not all lines of this IntSimplex are 45 degree
     */
    ...
    /**
     * Returns the simplex, which results from translating the lines of this
     * simplex by p_vector
     */
    @Override
    ...
    /**
     * Returns the smallest box with int coordinates containing all corners of
     * this simplex. The coordinates of the result will be Integer.MAX_VALUE, if
     * the simplex is not bounded
     */
    @Override
    ...
    /**
     * Calculates a bounding octagon of the Simplex. Returns null, if the
    ...
    public IntOctagon bounding_octagon() {
        if (precalculated_bounding_octagon == null) {
            ...
            if (Math.min(lx, ly) < -Limits.CRIT_INT
                    ...
                    || Math.max(lrx, urx) > Limits.CRIT_INT) // result is not bounded
            {
                return null;
            }
            precalculated_bounding_octagon = new IntOctagon((int) Math.floor(lx), (int) Math.floor(ly),
    ...
    /**
     * Returns the simplex offseted by p_with. If p_width > 0, the offset is to
     * the outer, else to the inner.
     */
    @Override
    ...
    /**
     * Returns this simplex enlarged by p_offset. The result simplex is
     * intersected with the by p_offset enlarged bounding octagon of this
     * simplex
     */
    @Override
    public Simplex enlarge(double p_offset) {
        ...
        IntOctagon bounding_oct = bounding_octagon();
    ...
    /**
     * if p_line is a borderline of this simplex the number of that edge is
    ...
    /**
     * Enlarges the simplex by removing the edge line with index p_no. The
    ...
    public Simplex remove_border_line(int p_no) {
        ...
        Line[] new_arr = new Line[arr.length - 1];
        System.arraycopy(arr, 0, new_arr, 0, p_no);
        System.arraycopy(arr, p_no + 1, new_arr, p_no, new_arr.length - p_no);
        return new Simplex(new_arr);
    }
    ...
    /**
     * cuts this simplex out of p_outer_simplex. Divides the resulting shape
     * into simplices along the minimal distance lines from the vertices of the
     * inner simplex to the outer simplex; Returns the convex pieces constructed
     * by this division.
     */
    @Override
    public Simplex[] cutout_from(Simplex p_outer_simplex) {
        ...
        Simplex[] result = result_list.toArray(Simplex[]::new);
        return result;
    }
    ...
    /**
     * Removes lines, which are redundant in the definition of the shape of this
     * simplex. Assumes that the lines of this simplex are sorted.
     */
    ...
    /**
     * For each corner of this inner simplex 1 or 2 perpendicular projections
     * onto lines of the outer simplex are constructed, so that the resulting
     * pieces after cutting out the inner simplex are convex. 2 projections may
     * be nessesary at sharp angle corners. Used in in the method cutout_from
     * with parametertype Simplex.
     */
    private Line[] calc_division_lines(int p_inner_corner_no, Simplex p_outer_simplex) {
        Line curr_inner_line = arr[p_inner_corner_no];
        Line prev_inner_line;
        if (p_inner_corner_no != 0) {
            prev_inner_line = arr[p_inner_corner_no - 1];
        } else {
            prev_inner_line = arr[arr.length - 1];
        }
