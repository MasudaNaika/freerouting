...
public class BatchOptRoute {
    ...
    private boolean opt_route_pass(int p_pass_no, boolean p_with_prefered_directions) {
        ...
        double trace_length_before = thread.hdlg.coordinate_transform.board_to_user(routing_board.cumulative_trace_length());
        thread.hdlg.screen_messages.set_post_route_info(via_count_before, trace_length_before);
        sorted_route_items = new ReadSortedRouteItems();
        min_cumulative_trace_length_before = calc_weighted_trace_length(routing_board);
        while (true) {
    ...
    private boolean opt_route_item(Item p_item, int p_pass_no, boolean p_with_prefered_directions) {
        ResourceBundle resources
                = ResourceBundle.getBundle("interactive.resources.InteractiveState", thread.hdlg.get_locale());
        String start_message = resources.getString("batch_optimizer") + " "
                + resources.getString("stop_message") + "        "
                + resources.getString("pass") + " "
                + Integer.toString(p_pass_no) + ": ";
        thread.hdlg.screen_messages.set_status_message(start_message);
        ...
        routing_board.remove_items(ripped_connections, false);
        for (int i = 0; i < p_item.net_count(); ++i) {
            routing_board.combine_traces(p_item.get_net_no(i));
        }
        int ripup_costs = thread.hdlg.get_settings().autoroute_settings.get_start_ripup_costs();
        ...
        BatchAutorouter.autoroute_passes_for_optimizing_item(thread, MAX_AUTOROUTE_PASSES,
                ripup_costs, p_with_prefered_directions);
        thread.hdlg.remove_ratsnest();
        int incomplete_count_after = thread.hdlg.get_ratsnest().incomplete_count();
        int via_count_after = routing_board.get_vias().size();
        double trace_length_after = calc_weighted_trace_length(routing_board);
        boolean route_improved = !thread.is_stop_requested() && (incomplete_count_after < incomplete_count_before
        ...
        if (route_improved) {
            ...
            double new_trace_length = thread.hdlg.coordinate_transform.board_to_user(routing_board.cumulative_trace_length());
            thread.hdlg.screen_messages.set_post_route_info(via_count_after, new_trace_length);
        } else {
