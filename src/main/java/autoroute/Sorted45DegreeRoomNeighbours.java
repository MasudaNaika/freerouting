...
public class Sorted45DegreeRoomNeighbours {
    ...
    private static Sorted45DegreeRoomNeighbours calculate_neighbours(ExpansionRoom p_room, int p_net_no,
        ...
        if (p_room instanceof IncompleteFreeSpaceExpansionRoom) {
            ...
            Freerouter.logInfo("Sorted45DegreeRoomNeighbours.calculate_neighbours: unexpected expansion room type");
            return null;
        }
    ...
    /**
     * Calculates an incomplete room for each edge side from p_from_side_no to
     * p_to_side_no.
     */
    private void calculate_edge_incomplete_rooms_of_obstacle_expansion_room(int p_from_side_no, int p_to_side_no, AutorouteEngine p_autoroute_engine) {
        if (!(from_room instanceof ObstacleExpansionRoom)) {
            Freerouter.logInfo("Sorted45DegreeRoomNeighbours.calculate_side_incomplete_rooms_of_obstacle_expansion_room: ObstacleExpansionRoom expected for from_room");
            return;
        }
        IntOctagon board_bounding_oct = p_autoroute_engine.board.get_bounding_box().bounding_octagon();
        IntPoint curr_corner = room_shape.corner(p_from_side_no);
        int curr_side_no = p_from_side_no;
        while (true) {
            int next_side_no = (curr_side_no + 1) % 8;
            IntPoint next_corner = room_shape.corner(next_side_no);
            if (!curr_corner.equals(next_corner)) {
                ...
                switch (curr_side_no) {
                    ...
                    case 7:
                        ...
                        break;
                    default:
                        Freerouter.logInfo("SortedOrthoganelRoomNeighbours.calculate_edge_incomplete_rooms_of_obstacle_expansion_room: curr_side_no illegal");
                        return;
                }
    ...
    private boolean try_remove_edge_line(int p_net_no, ShapeSearchTree p_autoroute_search_tree) {
        if (!(from_room instanceof IncompleteFreeSpaceExpansionRoom)) {
            return false;
        }
        ...
        if (!(curr_incomplete_room.get_shape() instanceof IntOctagon)) {
            Freerouter.logInfo("Sorted45DegreeRoomNeighbours.try_remove_edge_line: IntOctagon expected for room_shape type");
            return false;
        }
        ...
        for (int i = 0; i < 8; ++i) {
            if (!edge_interiour_touches_obstacle[i]) {
                FloatPoint prev_corner = room_shape.corner_approx(i);
                FloatPoint next_corner = room_shape.corner_approx(room_shape.next_no(i));
        ...
        if (try_remove_edge_lines) {
            ...
            IntOctagon enlarged_oct = remove_not_touching_border_lines(room_oct, edge_interiour_touches_obstacle);

            Collection<ExpansionDoor> door_list = completed_room.get_doors();
            ...
            for (ExpansionDoor curr_door : door_list) {
                ...
                if (curr_door.dimension == 2) {
                    CompleteExpansionRoom other_room = curr_door.other_room(completed_room);
                    {
    ...
    private void insert_incomplete_room(AutorouteEngine p_autoroute_engine, int p_lx, int p_ly, int p_rx, int p_uy,
        ...
        if (new_incomplete_room_shape.dimension() == 2) {
            IntOctagon new_contained_shape = room_shape.intersection(new_incomplete_room_shape);
            if (!new_contained_shape.is_empty()) {
                int door_dimension = new_contained_shape.dimension();
                if (door_dimension > 0) {
                    FreeSpaceExpansionRoom new_room
                            = p_autoroute_engine.add_incomplete_expansion_room(new_incomplete_room_shape, from_room.get_layer(), new_contained_shape);
                    ExpansionDoor new_door = new ExpansionDoor(completed_room, new_room, door_dimension);
                    completed_room.add_door(new_door);
    ...
    private void calculate_new_incomplete_rooms_for_obstacle_expansion_room(SortedRoomNeighbour p_prev_neighbour,
        ...
        calculate_edge_incomplete_rooms_of_obstacle_expansion_room(curr_from_side_no,
                curr_to_side_no, p_autoroute_engine);
    }

    private void calculate_new_incomplete_rooms(AutorouteEngine p_autoroute_engine) {
        IntOctagon board_bounding_oct = p_autoroute_engine.board.bounding_box.bounding_octagon();
        SortedRoomNeighbour prev_neighbour = sorted_neighbours.last();
        if (from_room instanceof ObstacleExpansionRoom && sorted_neighbours.size() == 1) {
        ...
        for (SortedRoomNeighbour next_neighbour : sorted_neighbours) {
            ...
            if (completed_room instanceof ObstacleExpansionRoom && sorted_neighbours.size() == 2) {
                ...
                if (from_room instanceof ObstacleExpansionRoom
                    ...
                    switch (next_neighbour.first_touching_side) {
                        ...
                        case 7:
                            ...
                            break;
                        default:
                            Freerouter.logInfo("Sorted45DegreeRoomNeighbour.calculate_new_incomplete: illegal touching side");
                            break;
                    }
                    insert_incomplete_room(p_autoroute_engine, lx, ly, rx, uy, ulx, lrx, llx, urx);
                }
    ...
    private class SortedRoomNeighbour implements Comparable<SortedRoomNeighbour> {
        ...
        /**
         * Creates a new instance of SortedRoomNeighbour and calculates the
         * first and last touching sides with the room shape. last_touching_side
         * will be -1, if sorting did not work because the room_shape is
         * contained in the neighbour shape.
         */
        public SortedRoomNeighbour(IntOctagon p_neighbour_shape, IntOctagon p_intersection) {
            ...
            int next_side_no = first_touching_side;
            while (true) {
        ...
        public int compareTo(SortedRoomNeighbour p_other) {
            if (first_touching_side > p_other.first_touching_side) {
                return 1;
            }
            if (first_touching_side < p_other.first_touching_side) {
                return -1;
            ...
            if (cmp_value == 0) {
                ...
                int this_touching_side_diff = (last_touching_side - first_touching_side + 8) % 8;
