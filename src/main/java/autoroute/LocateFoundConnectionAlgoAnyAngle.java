...
/**
 * Calculates from the backtrack list the location of the traces and vias, which
 * realize a connection found by the maze search algorithm.
 *
 * @author Alfons Wirtz
 */
class LocateFoundConnectionAlgoAnyAngle extends LocateFoundConnectionAlgo {
    ...
    protected Collection<FloatPoint> calculate_next_trace_corners() {
        ...
        if (current_to_door_index >= current_target_door_index) {
            if (current_to_door_index == current_target_door_index) {
                FloatPoint nearest_point = current_target_shape.nearest_point(current_from_point.round()).to_float();
        ...
        double trace_halfwidth_exact = ctrl.compensated_trace_half_width[current_trace_layer];
        ...
        if (current_from_point.side_of(door_left_corner, door_right_corner) != Side.ON_THE_RIGHT) {
            // the door is already crossed at from_point
        ...
        while (true) {
            ...
            if (door_left_corner != null && left_tangent_point == null) {
                if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                    Freerouter.logInfo("LocateFoundConnectionAlgo.calculate_next_trace_corner: left tangent point is null");
                }
            ...
            if (door_right_corner != null && right_tangent_point == null) {
                if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                    Freerouter.logInfo("LocateFoundConnectionAlgo.calculate_next_trace_corner: right tangent point is null");
                }
                right_tangent_point = door_right_corner;
            }
            if (left_tangent_point != null && right_tangent_point != null
                    && right_tangent_point.side_of(current_from_point, left_tangent_point) != Side.ON_THE_RIGHT) {
            ...
            BacktrackElement next_to_info = backtrack_array[curr_door_ind];
            ...
            if (current_from_point.side_of(next_left_corner, next_right_corner) != Side.ON_THE_RIGHT) {
                // the door may be already crossed at from_point
                ...
                if (next_left_corner == null && next_right_corner == null) {
                    ...
                    if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                        Freerouter.logInfo("LocateFoundConnectionAlgo.calculate_next_trace_corner: next door passed unexpected");
                    }
                    ++current_to_door_index;
                    result.add(current_from_point);
                    return result;
            ...
            {
                if (next_left_corner.side_of(current_from_point, door_right_corner) == Side.ON_THE_RIGHT) {
                    // bend to the right
                ...
                if (next_right_corner.side_of(current_from_point, door_left_corner) == Side.ON_THE_LEFT) {
                    // bend to the left
            ...
            if (door_right_corner != null && next_right_corner.side_of(current_from_point, door_right_corner) != Side.ON_THE_RIGHT) {
                FloatPoint curr_tangential_point = current_from_point.left_tangential_point(next_right_corner, trace_halfwidth_max);
                if (curr_tangential_point != null) {
                    FloatLine check_line = new FloatLine(current_from_point, curr_tangential_point);
            ...
            if (door_left_corner != null && next_left_corner.side_of(current_from_point, door_left_corner) != Side.ON_THE_LEFT) {
                FloatPoint curr_tangential_point = current_from_point.right_tangential_point(next_left_corner, trace_halfwidth_max);
                if (curr_tangential_point != null) {
                    FloatLine check_line = new FloatLine(current_from_point, curr_tangential_point);
        ...
        if (end_of_trace) {
            ...
            if (left_tangent_point != null && nearest_point.side_of(current_from_point, left_tangent_point) == Side.ON_THE_LEFT) {
                // The nearest target point is to the left of the visible range, add another corner
                new_door_ind = left_ind + 1;
                FloatPoint target_right_corner = current_target_shape.corner_approx(current_target_shape.index_of_right_most_corner(current_from_point));
                FloatPoint curr_corner = right_left_tangential_point(current_from_point, target_right_corner, door_left_corner, trace_halfwidth_max);
                if (curr_corner != null) {
                    result_corner = curr_corner;
                    end_of_trace = false;
                }
            } else if (right_tangent_point != null && nearest_point.side_of(current_from_point, right_tangent_point) == Side.ON_THE_RIGHT) {
                // The nearest target point is to the right of the visible range, add another corner
                FloatPoint target_left_corner = current_target_shape.corner_approx(current_target_shape.index_of_left_most_corner(current_from_point));
                new_door_ind = right_ind + 1;
                FloatPoint curr_corner = left_right_tangential_point(current_from_point, target_left_corner, door_right_corner, trace_halfwidth_max);
                if (curr_corner != null) {
                    result_corner = curr_corner;
                    end_of_trace = false;
                }
        ...
        FloatLine check_line = new FloatLine(current_from_point, result_corner);
        ...
        for (int i = check_from_door_index; i < new_door_ind; ++i) {
            FloatPoint curr_left_corner = calc_door_left_corner(backtrack_array[i]);
            ...
            if (Math.abs(curr_dist) < trace_halfwidth_middle) {
                ...
                if (curr_corrected_result != null) {
                    if (corrected_result == null
                            || curr_corrected_result.side_of(current_from_point, corrected_result) == Side.ON_THE_RIGHT) {
                        corrected_door_ind = i;
                        corrected_result = curr_corrected_result;
                    }
                }
            }
            FloatPoint curr_right_corner = calc_door_right_corner(backtrack_array[i]);
            ...
            if (Math.abs(curr_dist) < trace_halfwidth_middle) {
                ...
                if (curr_corrected_result != null) {
                    if (corrected_result == null
                            || curr_corrected_result.side_of(current_from_point, corrected_result) == Side.ON_THE_LEFT) {
                        corrected_door_ind = i;
                        corrected_result = curr_corrected_result;
                    }
                }
     ...
     * p_next_corner and radius 2 * p_dist is constructed. The second line is
     * than translated by the distance p_dist to the left. Returned is the
     * intersection of the first and the second line.
     */
    private FloatPoint right_turn_next_corner(FloatPoint p_from_corner, double p_dist, FloatPoint p_to_corner, FloatPoint p_next_corner) {
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo.right_turn_next_corner: left tangential point is null");
            }
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo.right_turn_next_corner: right tangential point is null");
            }
            return p_from_corner;
        }
    ...
    /**
     * Calculates as first line the right side tangent from p_from_corner to the
     * circle with center p_to_corner and radius p_dist. As second line the left
     * side tangent from p_to_corner to the circle with center p_next_corner and
     * radius 2 * p_dist is constructed. The second line is than translated by
     * the distance p_dist to the right. Returned is the intersection of the
    ...
    private FloatPoint left_turn_next_corner(FloatPoint p_from_corner, double p_dist, FloatPoint p_to_corner, FloatPoint p_next_corner) {
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo.left_turn_next_corner: right tangential point is null");
            }
            return p_from_corner;
        }
        FloatLine first_line = new FloatLine(p_from_corner, curr_tangential_point);
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo.left_turn_next_corner: left tangential point is null");
            }
            return p_from_corner;
        }
        FloatLine second_line = new FloatLine(p_to_corner, curr_tangential_point);
    ...
    /**
     * Calculates the right tangential line from p_from_point and the left
     * tangential line from p_to_point to the circle with center p_center and
     * radius p_dist. Returns the intersection of the 2 lines.
     */
    private FloatPoint right_left_tangential_point(FloatPoint p_from_point, FloatPoint p_to_point, FloatPoint p_center, double p_dist) {
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo. right_left_tangential_point: right tangential point is null");
            }
            return null;
        }
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo. right_left_tangential_point: left tangential point is null");
            }
            return null;
        }
    ...
    /**
     * Calculates the left tangential line from p_from_point and the right
     * tangential line from p_to_point to the circle with center p_center and
     * radius p_dist. Returns the intersection of the 2 lines.
     */
    private FloatPoint left_right_tangential_point(FloatPoint p_from_point, FloatPoint p_to_point, FloatPoint p_center, double p_dist) {
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo. left_right_tangential_point: left tangential point is null");
            }
            return null;
        }
        FloatLine first_line = new FloatLine(p_from_point, curr_tangential_point);
        ...
        if (curr_tangential_point == null) {
            if (test_level.ordinal() >= TestLevel.ALL_DEBUGGING_OUTPUT.ordinal()) {
                Freerouter.logInfo("LocateFoundConnectionAlgo. left_right_tangential_point: right tangential point is null");
            }
            return null;
        }
        FloatLine second_line = new FloatLine(p_to_point, curr_tangential_point);
