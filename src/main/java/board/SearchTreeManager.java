...
public class SearchTreeManager {
     ...
     * normally the case, if there exist only the clearance classes null and
     * default in the clearance matrix.
     */
    ...
    public void set_clearance_compensation_used(boolean p_value) {
        if (clearance_compensation_used == p_value) {
            return;
        }
        ...
        default_tree = new ShapeSearchTree(FortyfiveDegreeBoundingDirections.INSTANCE, board, compensated_clearance_class_no);
        compensated_search_trees.add(default_tree);
    ...
    public void clearance_value_changed() {
        ...
        compensated_search_trees.removeIf(curr_tree
                -> curr_tree.compensated_clearance_class_no != default_tree.compensated_clearance_class_no);
     ...
     * p_clearance_vlass_no. Initialized the tree, if it is not yet allocated.
     */
    public ShapeSearchTree get_autoroute_tree(int p_clearance_class_no) {
        ...
        boolean fast_algorithm = !board.rules.get_slow_autoroute_algorithm();
        if (fast_algorithm && board.rules.get_trace_angle_restriction() == AngleRestriction.NINETY_DEGREE) {
            curr_autoroute_tree = new ShapeSearchTree90Degree(board, p_clearance_class_no);
        } else if (fast_algorithm && board.rules.get_trace_angle_restriction() == AngleRestriction.FORTYFIVE_DEGREE) {
            curr_autoroute_tree = new ShapeSearchTree45Degree(board, p_clearance_class_no);
        } else {
            curr_autoroute_tree = new ShapeSearchTree(FortyfiveDegreeBoundingDirections.INSTANCE, board, p_clearance_class_no);
        }
    ...
    /**
     * Clears all compensated trees used in the autoroute algorithm apart from
     * the default tree.
     */
    public void reset_compensated_trees() {
        compensated_search_trees.removeIf(curr_tree -> curr_tree != default_tree);
    }
