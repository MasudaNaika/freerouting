...
public class LineSegment implements Serializable {
    ...
    public Simplex to_simplex() {
        ...
        if (end_point().side_of(start) == Side.ON_THE_RIGHT) {
        ...
        if (start_point().side_of(end) == Side.ON_THE_RIGHT) {
    ...
    public boolean contains(Point p_point) {
        ...
        Side start_point_side = perpendicular_line.side_of(start_point());
        Side end_point_side = perpendicular_line.side_of(end_point());
    ...
    public LineSegment change_length_approx(double p_new_length) {
        ...
        LineSegment result = new LineSegment(start, middle, new_end_line);
        return result;
    }
    ...
    public Line[] intersection(LineSegment p_other) {
        ...
        if (start_point_side == Side.COLLINEAR && end_point_side == Side.COLLINEAR) {
            ...
            LineSegment this_sorted = sort_endpoints_in_x_y();
        ...
        if (start_point_side == end_point_side
                || p_other.start_point().side_of(middle) == p_other.end_point().side_of(middle)) {
    ...
    public boolean intersects(LineSegment p_other) {
        Line[] intersections = intersection(p_other);
    ...
    /**
     * Constructs an approximation of this line segment by orthogonal stairs
     * with integer coordinates. The length of the stairs will be at most
     * p_stair_width. If p_to_the_right, the stairs will be to the right of this
     * line segment, else to the left.
     */
    public IntPoint[] stair_approximation(double p_width, boolean p_to_the_right) {
        IntPoint start_point = start_point().to_float().round();
        IntPoint end_point = end_point().to_float().round();
    ...
    /**
     * Constructs an approximation of this line segment by 45 degree stairs with
     * integer coordinates. The length of the stairs will be at most
     * p_stair_width. If p_to_the_right, the stairs will be to the right of this
     * line segment, else to the left.
     */
    ...
    public int[] border_intersections(TileShape p_shape) {
        ...
        if (!bounding_box().intersects(p_shape.bounding_box())) {
            return empty_result;
        }
                ...
                if (prev_line_side_of_is != Side.ON_THE_LEFT && next_line_side_of_is != Side.ON_THE_LEFT) {
                    ...
                    if (next_line_side_of_is == Side.COLLINEAR) {
                        ...
                        Side next_next_corner_side
                                = middle.side_of(next_next_corner);
    ...
    /**
     * Inverts the direction of middle, if start_point() has a bigger x
    ...
    public LineSegment sort_endpoints_in_x_y() {
        ...
        if (swap_endlines) {
            result = new LineSegment(end, middle, start);
