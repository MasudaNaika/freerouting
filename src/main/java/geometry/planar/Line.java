...
public class Line implements Comparable<Line>, Serializable {
    ...
    /**
     * Returns true, if this and p_other define the same line. Is designed for
    ...
    /**
     * The function returns Side.ON_THE_LEFT, if this Line is on the left of
     * p_point, Side.ON_THE_RIGHT, if this Line is on the right of p_point and
    ...
    /**
     * Returns Side.COLLINEAR, if p_point is on the line with tolerance
     * p_tolerance. Otherwise Side.ON_THE_LEFT, if this line is on the left of
     * p_point, or Side.ON_THE_RIGHT, if this line is on the right of p_point,
     */
     ...
     * of p_1 and p_2, Side.ON_THE_RIGHT, if this line is on the right of the
     * intersection, and Side.COLLINEAR, if all 3 lines intersect in exacly 1
     * point.
     */
    public Side side_of_intersection(Line p_1, Line p_2) {

        FloatPoint intersection_approx = p_1.intersection_approx(p_2);
        Side result = side_of(intersection_approx, 1.0);
    ...
    public boolean is_on_the_left(TileShape p_tile) {
        for (int i = 0; i < p_tile.border_line_count(); ++i) {
            if (side_of(p_tile.corner(i)) == Side.ON_THE_RIGHT) {
                return false;
            }
        }
        return true;
    ...
    public boolean is_on_the_right(TileShape p_tile) {
        for (int i = 0; i < p_tile.border_line_count(); ++i) {
            if (side_of(p_tile.corner(i)) == Side.ON_THE_LEFT) {
                return false;
            }
        }
        return true;
    ...
    /**
     * returns true, if the 2 lines defins the same set of points, but may have
     * opposite directions
     */
    ...
    /**
     * Returns the line defining the same set of points, but with opposite
     * direction
     */
    ...
    public Point intersection(Line p_other) {
        ...
        if (delta_1.x == 0) // this line is vertical
        {
            ...
            {
                return new IntPoint(((IntPoint) a).x, ((IntPoint) p_other.a).y);
            }
            if (delta_2.x == delta_2.y) // other line is right diagonal
            {
                int this_x = ((IntPoint) a).x;
                IntPoint other_a = (IntPoint) p_other.a;
        ...
        } else if (delta_1.y == 0) // this line is horizontal
        {
            ...
            {
                return new IntPoint(((IntPoint) p_other.a).x, ((IntPoint) a).y);
            }
            if (delta_2.x == delta_2.y) // other line is right diagonal
            {
                int this_y = ((IntPoint) a).y;
                IntPoint other_a = (IntPoint) p_other.a;
     ...
     * Integer.MAX_VALUE. Useful in situations ehere performance is more
    ...
    /**
     * translates the line perpendicular at about p_dist. If p_dist > 0, the
    ...
    public boolean is_parallel(Line p_other) {
        return direction().side_of(p_other.direction()) == Side.COLLINEAR;
    }
    ...
    /**
     * A line l_1 is defined bigger than a line l_2, if the direction of l_1 is
     * bigger than the direction of l_2. Implements the comparable interface.
     * Throws a cast exception, if p_other is not a Line. Fast implementation
     * only for lines consisting of IntPoints because of critical performance
     */
    @Override
    ...
    /**
     * Calculates an approximation of the function value of this line at p_x, if
    ...
    /**
     * Calculates an approximation of the function value in y of this line at
     * p_y, if the line is not horizontal.
     */
    ...
    /**
     * Calculates the direction from p_from_point to the nearest point on this
     * line to p_fro_point. Returns null, if p_from_point is contained in this
     * line.
     */
    public Direction perpendicular_direction(Point p_from_point) {
        Side line_side = side_of(p_from_point);
        if (line_side == Side.COLLINEAR) {
            return null;
        }
        Direction dir1 = direction().turn_45_degree(2);
        Direction dir2 = direction().turn_45_degree(6);
