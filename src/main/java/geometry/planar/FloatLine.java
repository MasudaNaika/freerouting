 ...
 * defined for FloatLines. If exactnesss is needed, use the class Line instead.
 *
 * @author Alfons Wirtz
 */
public class FloatLine {
    ...
    public FloatLine opposite() {
        return new FloatLine(b, a);
    }

    public FloatLine adjust_direction(FloatLine p_other) {
        if (b.side_of(a, p_other.a) == p_other.b.side_of(a, p_other.a)) {
            return this;
        }
        return opposite();
    }
    ...
    /**
     * Calculates the intersection of this line with p_other. Returns null, if
    ...
    public FloatLine translate(double p_dist) {
        ...
        if (dxdx <= dydy) {
            ...
            new_a = new FloatPoint(a.x - rel_x, a.y);
        } else {
            // translate along the  y axis
            double rel_y = (p_dist * lenght) / dx;
            new_a = new FloatPoint(a.x, a.y + rel_y);
        }
    ...
    /**
     * Returns the distance of p_point to the nearest point of this line betweem
     * this.a and this.b.
     */
    public double segment_distance(FloatPoint p_point) {
        FloatPoint projection = perpendicular_projection(p_point);
        double result;
        if (projection.is_contained_in_box(a, b, 0.01)) {
    ...
    /**
     * Returns the perpendicular projection of p_line_segment onto this oriented
     * line segment, Returns null, if the projection is empty.
     */
    public FloatLine segment_projection(FloatLine p_line_segment) {
        if (b.scalar_product(a, p_line_segment.a) < 0) {
            return null;
        }
        if (a.scalar_product(b, p_line_segment.b) < 0) {
            return null;
        }
        FloatPoint projected_a;
        if (a.scalar_product(b, p_line_segment.a) < 0) {
            projected_a = a;
        } else {
            projected_a = perpendicular_projection(p_line_segment.a);
            if (Math.abs(projected_a.x) >= Limits.CRIT_INT || Math.abs(projected_a.y) >= Limits.CRIT_INT) {
                return null;
            }
        }
        FloatPoint projected_b;
        if (b.scalar_product(a, p_line_segment.b) < 0) {
            projected_b = b;
        } else {
            projected_b = perpendicular_projection(p_line_segment.b);
        }
     ...
     * by moving p_line_segment perpendicular into the direction of this line
     * segmant Returns null, if the projection is empty or p_line_segment.a ==
     * p_line_segment.b
     */
    public FloatLine segment_projection_2(FloatLine p_line_segment) {
        if (p_line_segment.a.scalar_product(p_line_segment.b, b) <= 0) {
            return null;
        }
        if (p_line_segment.b.scalar_product(p_line_segment.a, a) <= 0) {
            return null;
        }
        FloatPoint projected_a;
        if (p_line_segment.a.scalar_product(p_line_segment.b, a) < 0) {
        ...
        if (p_line_segment.b.scalar_product(p_line_segment.a, b) < 0) {
            FloatLine curr_perpendicular_line
    ...
    /**
     * Shrinks this line on both sides by p_value. The result will contain at
     * least the gravity point of the line.
     */
    ...
    /**
     * Calculates the nearest point on this line to p_from_point between this.a
     * and this.b.
     */
    public FloatPoint nearest_segment_point(FloatPoint p_from_point) {
        FloatPoint projection = perpendicular_projection(p_from_point);
        if (projection.is_contained_in_box(a, b, 0.01)) {
            return projection;
        }
        // Now the projection is outside the line segment.
        FloatPoint result;
        if (p_from_point.distance_square(a) <= p_from_point.distance_square(b)) {
            result = a;
        } else {
            result = b;
        }
    ...
    /**
     * Divides this line segment into p_count line segments of nearly equal
     * length. and at most p_max_section_length.
     */
