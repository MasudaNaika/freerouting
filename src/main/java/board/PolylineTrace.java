...
public class PolylineTrace extends Trace implements Serializable {
    ...
    public IntBox bounding_box() {
        IntBox result = lines.bounding_box();
    ...
    public void rotate_approx(double p_angle_in_degree, FloatPoint p_pole) {
        lines = lines.rotate_approx(Math.toRadians(p_angle_in_degree), p_pole);
    }
    ...
    public void change_placement_side(IntPoint p_pole) {
        ...
        if (board != null) {
            set_layer(board.get_layer_count() - get_layer() - 1);
        }
        clear_derived_data();
    }
     ...
     * somthing has been combined. This trace will be the combined trace, so
     * that only other traces may be deleted.
     */
    @Override
    ...
    private boolean combine_at_start(boolean p_ignore_areas) {
        ...
        if (p_ignore_areas) {
            ...
            contacts.removeIf(item -> item instanceof ConductionArea);
        }
        if (contacts.size() != 1) {
            return false;
        }
    ...
    /**
     * looks, if this trace can be combined at its last point with another
     * trace. Returns true, if somthing was combined. The corners of the other
     * trace will be inserted at the end of thie trace. In case of combine the
     * other trace will be deleted and this trace will remain.
     */
        ...
        for (Item curr_ob : contacts) {
            if (curr_ob instanceof PolylineTrace) {
                ...
                if (other_trace.get_layer() == get_layer() && other_trace.nets_equal(this)
                        ...
                        && other_trace.get_fixed_state() == get_fixed_state()) {
     ...
     * intersection points. In case of an overlaps, the traces are split at
     * their first and their last common point. Returns the pieces resulting
     * from splitting. Found cycles are removed. If nothing is split, the result
     * will contain just this Trace. If p_clip_shape != null, the split may be
    ...
    public Collection<PolylineTrace> split(IntOctagon p_clip_shape) {
        ...
        for (int i = 0; i < lines.arr.length - 2; ++i) {
            if (p_clip_shape != null) {
                LineSegment curr_segment = new LineSegment(lines, i + 1);
                if (!p_clip_shape.intersects(curr_segment.bounding_box())) {
                    continue;
                }
            }
            TileShape curr_shape = get_tree_shape(default_tree, i);
            LineSegment curr_line_segment = new LineSegment(lines, i + 1);
            ...
            while (it.hasNext()) {
                ...
                } else if (!is_user_fixed() && (found_item instanceof ConductionArea)) {
                    boolean ignore_areas = false;
                    if (net_no_arr.length > 0) {
                        rules.Net curr_net = board.rules.nets.get(net_no_arr[0]);
                        if (curr_net != null && curr_net.get_class() != null) {
                            ignore_areas = curr_net.get_class().get_ignore_cycles_with_areas();
                        }
                    }
                    if (!ignore_areas && get_start_contacts().contains(found_item)
                            && get_end_contacts().contains(found_item)) {
    ...
    private boolean split_inside_drill_pad_prohibited(int p_line_no, Line p_line) {
        ...
        Point intersection = lines.arr[p_line_no].intersection(p_line);
        Collection<Item> overlap_items = board.pick_items(intersection, get_layer(), null);
    ...
    /**
     * Splits this trace into two at p_point. Returns the 2 pieces of the
     * splitted trace, or null if nothing was splitted because for example
     ...
     * Returns true, if something was changed. If p_clip_shape != null,
     * splitting is restricted to p_clip_shape.
     */
    public boolean normalize(IntOctagon p_clip_shape) {
        boolean observers_activated = false;
        BasicBoard routing_board = board;
        if (board != null) {
        ...
        Collection<PolylineTrace> split_pieces = split(p_clip_shape);
    ...
    public boolean pull_tight(PullTightAlgo p_pull_tight_algo) {
        ...
        if (net_no_arr.length > 0) {
            if (!board.rules.nets.get(net_no_arr[0]).get_class().get_pull_tight()) {
                return false;
            }
        }
        Polyline new_lines
                = p_pull_tight_algo.pull_tight(lines, get_layer(), get_half_width(), net_no_arr, clearance_class_no(),
                        touching_pins_at_end_corners());
        if (new_lines != lines) {
            change(new_lines);
            return true;
        }
        AngleRestriction angle_restriction = board.rules.get_trace_angle_restriction();
        if (angle_restriction != AngleRestriction.NINETY_DEGREE && board.rules.get_pin_edge_to_turn_dist() > 0) {
    ...
    public boolean pull_tight(boolean p_own_net_only, int p_pull_tight_accuracy, Stoppable p_stoppable_thread) {
        ...
        PullTightAlgo pull_tight_algo
                = PullTightAlgo.get_instance((RoutingBoard) board, opt_net_no_arr,
    ...
    public boolean check_connection_to_pin(boolean p_at_start) {
        ...
        Collection<Pin.TraceExitRestriction> trace_exit_restrictions = contact_pin.get_trace_exit_restrictions(get_layer());
        ...
        if (p_at_start) {
            ...
            prev_end_corner = lines.corner(1);
        } else {
            end_corner = last_corner();
            prev_end_corner = lines.corner(lines.corner_count() - 2);
        }
        ...
        final double edge_to_turn_dist = board.rules.get_pin_edge_to_turn_dist();
        if (edge_to_turn_dist < 0) {
            return false;
        }
        double end_line_length = end_corner.to_float().distance(prev_end_corner.to_float());
        double curr_clearance = board.clearance_value(clearance_class_no(), contact_pin.clearance_class_no(), get_layer());
    ...
    /**
     * Tries to correct a connection restriction of this trace. If p_at_start,
     * the start of the trace polygon is corrected, else the end. Returns true,
     * if this trace was changed.
     */
    public boolean correct_connection_to_pin(boolean p_at_start, AngleRestriction p_angle_restriction) {
        if (check_connection_to_pin(p_at_start)) {
            return false;
        }
        ...
        Shape pin_shape = contact_pin.get_shape(get_layer() - contact_pin.first_layer());
        ...
        double curr_clearance = board.clearance_value(clearance_class_no(), contact_pin.clearance_class_no(), get_layer());
        double add_width = Math.max(edge_to_turn_dist, curr_clearance + 1);
        TileShape offset_pin_shape = (TileShape) ((TileShape) pin_shape).offset(get_half_width() + add_width);
        ...
        if (!board.check_polyline_trace(border_polyline, get_layer(),
                get_half_width(), net_no_arr, clearance_class_no())) {
            return false;
        }
    ...
    /**
     * Looks, if an other pin connection restriction fits better than the
     * current connection restriction and changes this trace in this case. If
     * p_at_start, the start of the trace polygon is changed, else the end.
    ...
    public boolean swap_connection_to_pin(boolean p_at_start) {
        ...
        Direction nearest_pin_exit_direction
                = contact_pin.calc_nearest_exit_restriction_direction(combined_polyline, get_half_width(), get_layer());
