...
public class SnapShot implements Serializable {
    ...
    private SnapShot(String p_name, BoardHandling p_board_handling) {
        name = p_name;
        settings = new Settings(p_board_handling.settings);
        interactive_state_no = get_no(p_board_handling.interactive_state);
        graphics_context = new boardgraphics.GraphicsContext(p_board_handling.graphics_context);
        viewport_position = new java.awt.Point(p_board_handling.get_panel().get_viewport_position());
        subwindow_filters = p_board_handling.get_panel().board_frame.get_snapshot_subwindow_selections();
    }
    ...
    public java.awt.Point copy_viewport_position() {
        ...
        return new java.awt.Point(viewport_position);
    }
    ...
    public void go_to(interactive.BoardHandling p_board_handling) {
        interactive.SnapShot.Attributes snapshot_attributes = settings.snapshot_attributes;

        if (snapshot_attributes.object_visibility) {
            p_board_handling.graphics_context.color_intensity_table
                    = new boardgraphics.ColorIntensityTable(graphics_context.color_intensity_table);
        }
        if (snapshot_attributes.layer_visibility) {
            p_board_handling.graphics_context.set_layer_visibility_arr(graphics_context.copy_layer_visibility_arr());
        }

        if (snapshot_attributes.interactive_state) {
            p_board_handling.set_interactive_state(get_interactive_state(p_board_handling, p_board_handling.logfile));
        }
        if (snapshot_attributes.selection_layers) {
            p_board_handling.settings.select_on_all_visible_layers = settings.select_on_all_visible_layers;
        }
        if (snapshot_attributes.selectable_items) {
            p_board_handling.settings.item_selection_filter = new board.ItemSelectionFilter(settings.item_selection_filter);
        }
        if (snapshot_attributes.current_layer) {
            p_board_handling.settings.layer = settings.layer;
        }
        if (snapshot_attributes.rule_selection) {
            p_board_handling.settings.manual_rule_selection = settings.manual_rule_selection;
        }
        if (snapshot_attributes.manual_rule_settings) {
            p_board_handling.settings.manual_trace_clearance_class = settings.manual_trace_clearance_class;
            p_board_handling.settings.manual_via_rule_index = settings.manual_via_rule_index;
            System.arraycopy(settings.manual_trace_half_width_arr, 0,
        ...
        if (snapshot_attributes.push_and_shove_enabled) {
            p_board_handling.settings.push_enabled = settings.push_enabled;
        }
        if (snapshot_attributes.drag_components_enabled) {
            p_board_handling.settings.drag_components_enabled = settings.drag_components_enabled;
        }
        if (snapshot_attributes.pull_tight_region) {
            p_board_handling.settings.trace_pull_tight_region_width = settings.trace_pull_tight_region_width;
        }
        if (snapshot_attributes.component_grid) {
            p_board_handling.settings.horizontal_component_grid = settings.horizontal_component_grid;
            p_board_handling.settings.vertical_component_grid = settings.vertical_component_grid;
        }
