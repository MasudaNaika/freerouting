...
public class UndoableObjects implements Serializable {
    ...
    public UndoableObjects() {
        ...
        deleted_objects_stack = new LinkedList<>();
    }

    /**
     * Returns an iterator for sequential reading of the object list. Use it
     * together with this.read_object().
     */
    ...
    /**
     * Reads the next object in this list. Returns null, if the list is
     * exhausted. p_it must be created by start_read_object.
     */
    public UndoableObjects.Storable read_object(Iterator<UndoableObjectNode> p_it) {
        while (p_it.hasNext()) {
            ...
            if (curr_node != null && curr_node.level <= stack_level) {
    ...
    public boolean delete(UndoableObjects.Storable p_object) {
        ...
        if (deleted_objects_stack.isEmpty()) {
            ...
            curr_delete_list = deleted_objects_stack.getLast();
        }
        ...
        if (curr_delete_list != null) {
            if (object_node.level < stack_level) {
    ...
    /**
     * Restores the situation before the last snapshot. Outputs the cancelled
     * and the restored objects (if != null) to enable the calling function to
     * take additional actions needed for these objects. Returns false, if no
     * more undo is possible
     */
    public boolean undo(Collection<UndoableObjects.Storable> p_cancelled_objects, Collection<UndoableObjects.Storable> p_restored_objects) {
        ...
        Collection<UndoableObjectNode> curr_delete_list = deleted_objects_stack.get(stack_level - 1);
        for (UndoableObjectNode curr_deleted_node : curr_delete_list) {
            objects.put(curr_deleted_node.object, curr_deleted_node);
            if (p_restored_objects != null) {
    ...
    /**
     * Restores the situation before the last undo. Outputs the cancelled and
     * the restored objects (if != null) to enable the calling function to take
     * additional actions needed for these objects. Returns false, if no more
     * redo is possible.
     */
    public boolean redo(Collection<UndoableObjects.Storable> p_cancelled_objects, Collection<UndoableObjects.Storable> p_restored_objects) {
        if (stack_level >= deleted_objects_stack.size()) {
        ...
        for (UndoableObjectNode curr_deleted_node : curr_delete_list) {
            while (curr_deleted_node.redo_object != null
                    && curr_deleted_node.redo_object.level <= stack_level) {
     ...
     * cannot be restored any more. Returns false, if no more snapshot could be
     * popped.
     */
    public boolean pop_snapshot() {
        ...
        for (UndoableObjectNode curr_node : objects.values()) {
            if (curr_node.level == stack_level - 1
                    ...
                    && curr_node.redo_object.level == stack_level) {
        ...
        if (deleted_objects_stack_size >= 2) {
            ...
            Collection<UndoableObjectNode> from_delete_list = deleted_objects_stack.get(deleted_objects_stack_size - 1);
            Collection<UndoableObjectNode> to_delete_list = deleted_objects_stack.get(deleted_objects_stack_size - 2);
            for (UndoableObjectNode curr_deleted_node : from_delete_list) {
                if (curr_deleted_node.level < stack_level - 1) {
    ...
    /**
     * Must be callel before p_object will be modified after a snapshot for the
     ...
     */
    private LinkedList<Collection<UndoableObjectNode>> deleted_objects_stack;
