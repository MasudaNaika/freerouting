...
public class MazeSearchAlgo {
     ...
     * section number of the found destination is returned, from where the whole
     * found connection can be backtracked. Otherwise the return value will be
     * null.
     */
    public Result find_connection() {
        ...
        return new Result(destination_door, section_no_of_destination_door);
    }

    /**
     * Expands the next element in the maze expansion list. Returns false, if
     * the expansion list is exhausted or the destination is reached.
     */
    ...
    /**
     * Expands the other door section of the room. Returns true, if the from
     * door section has to be occupied, and false, if the occupation for is
     * delayed.
     */
    private boolean expand_to_room_doors(MazeListElement p_list_element) {
        ...
        autoroute_engine.complete_neigbour_rooms(p_list_element.next_room);

        FloatPoint shape_entry_middle = p_list_element.shape_entry.a.middle_point(p_list_element.shape_entry.b);

        if (ctrl.with_neckdown && p_list_element.door instanceof TargetItemExpansionDoor) {
        ...
        if (p_list_element.next_room instanceof FreeSpaceExpansionRoom) {
            ...
            if (!p_list_element.already_checked) {
                ...
                if (ripup_costs != ALREADY_RIPPED_COSTS && next_room_is_thick) {
                    ...
                    if (!curr_door_is_small && ctrl.max_shove_trace_recursion_depth > 0 && obstacle_item instanceof board.PolylineTrace) {
        ...
        if (ctrl.vias_allowed && !(p_list_element.door instanceof ExpansionDrill)) {
            if ((something_expanded || next_room_is_thick) && p_list_element.next_room instanceof CompleteFreeSpaceExpansionRoom) {
                ...
                Collection<DrillPage> overlapping_drill_pages
                        = autoroute_engine.drill_page_array.overlapping_pages(p_list_element.next_room.get_shape());
                {
                ...
                if (curr_obstacle_item instanceof board.Via) {
                    ...
                    ExpansionDrill via_drill_info = curr_via.get_autoroute_drill_info(autoroute_engine.autoroute_search_tree);
    ...
    /**
     * Expand the target doors of the room. Returns true, if at leat 1 target
     * door was expanded
     */
    private boolean expand_to_target_doors(MazeListElement p_list_element,
        ...
        for (TargetItemExpansionDoor to_door : p_list_element.next_room.get_target_doors()) {
            ...
            TileShape target_shape = ((Connectable) to_door.item).get_trace_connection_shape(autoroute_engine.autoroute_search_tree,
    ...
    private void expand_to_drill(ExpansionDrill p_drill, MazeListElement p_from_element, int p_add_costs) {
        int layer = p_from_element.next_room.get_layer();
        int trace_half_width = ctrl.compensated_trace_half_width[layer];
    ...
    /**
     * A drill page is inserted between an expansion roomm and the drill to
     * expand in order to prevent performance problems with rooms with big
     * shapes containing many drills.
     */
    private void expand_to_drill_page(DrillPage p_drill_page, MazeListElement p_from_element) {
        ...
        double sorting_value
                = expansion_value + nearest_point.weighted_distance(from_element_shape_entry_middle,
                        ctrl.trace_costs[layer].horizontal, ctrl.trace_costs[layer].vertical) + destination_distance.calculate(nearest_point, layer);
        MazeListElement new_element
    ...
    private void expand_to_drills_of_page(MazeListElement p_from_element) {
        ...
        Collection<ExpansionDrill> drill_list = drill_page.get_drills(autoroute_engine, ctrl.attach_smd_allowed);
    ...
    private void expand_to_other_layers(MazeListElement p_list_element) {
        ...
        if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom) {
            ...
            if (!ctrl.via_rule.contains_padstack(curr_obstacle_padstack) || curr_obstacle_item.clearance_class_no() != ctrl.via_clearance_class) {
                return;
            }
    ...
    /**
     * Initializes the maze search algorithm. Returns false if the
     * initialisation failed.
     */
    private boolean init(Set<Item> p_start_items, Set<Item> p_destination_items) {
        reduce_trace_shapes_at_tie_pins(p_start_items, ctrl.net_no, search_tree);
        reduce_trace_shapes_at_tie_pins(p_destination_items, ctrl.net_no, search_tree);
        ...
        for (Item curr_item : p_destination_items) {
            ...
            for (int i = 0; i < curr_item.tree_shape_count(search_tree); ++i) {
                TileShape curr_tree_shape = curr_item.get_tree_shape(search_tree, i);
        ...
        if (!destination_ok && ctrl.is_fanout) {
            ...
            IntBox board_bounding_box = autoroute_engine.board.bounding_box;
            destination_distance.join(board_bounding_box, 0);
            destination_distance.join(board_bounding_box, ctrl.layer_count - 1);
            destination_ok = true;
        }
        ...
        for (Item curr_item : p_start_items) {
            ...
            if (curr_item instanceof Connectable) {
                for (int i = 0; i < curr_item.tree_shape_count(search_tree); ++i) {
                    ...
                    IncompleteFreeSpaceExpansionRoom new_start_room = autoroute_engine
                            .add_incomplete_expansion_room(null, curr_item.shape_layer(i), contained_shape);
        ...
        if (autoroute_engine.maintain_database) {
            // add the completed start rooms carried over from the last autoroute to the start rooms.
            completed_start_rooms.addAll(autoroute_engine.get_rooms_with_target_items(p_start_items));
        }
    ...
    private boolean room_shape_is_thick(ObstacleExpansionRoom p_obstacle_room) {
        ...
        if (obstacle_item instanceof board.Trace) {
            obstacle_half_width = ((board.Trace) obstacle_item).get_half_width()
                    + search_tree.clearance_compensation_value(obstacle_item.clearance_class_no(), layer);
     ...
     * > 0, if the room is ripped and -1, if no ripup was possible. If the
     * previous room was also ripped and contained the same item or an item of
     * the same connection, the result will be equal to ALREADY_RIPPED_COSTS
     */
    private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small) {
        ...
        if (p_obstacle_item instanceof board.Trace) {
            ...
            if (!ctrl.remove_unconnected_vias) {
            ...
            boolean look_if_fanout_via = !ctrl.remove_unconnected_vias;
        ...
        double ripup_cost = ctrl.ripup_costs * cost_factor;
        double detour = 1;
        ...
        boolean randomize = ctrl.ripup_pass_no >= 4 && ctrl.ripup_pass_no % 3 != 0;
        if (randomize) {
     ...
     * no door was expanded. In this case occupation of the door_section by
     * ripup can be delayed to allow shoving the room fromm a different door
     * section
     */
    private boolean shove_trace_room(MazeListElement p_list_element, ObstacleExpansionRoom p_obstacle_room) {
        ...
        if (p_list_element.adjustment != MazeSearchElement.Adjustment.RIGHT) {
            ...
            if (MazeShoveTraceAlgo.check_shove_trace_line(p_list_element, p_obstacle_room,
                    autoroute_engine.board, ctrl, false, left_to_door_section_list)) {
                result = true;
            }
        ...
        if (p_list_element.adjustment != MazeSearchElement.Adjustment.LEFT) {
            ...
            if (MazeShoveTraceAlgo.check_shove_trace_line(p_list_element, p_obstacle_room,
                    autoroute_engine.board, ctrl, true, right_to_door_section_list)) {
                result = true;
            }
    ...
    /**
     * Checks, if the next roomm contains a destination pin, where evtl.
     * neckdown is necessary. Return the neck down width in this case, or 0, if
     * no such pin waas found,
     */
     ...
     * small. If p_ignore_item != null, p_ignore_item and all other items
     * directly connected to p_ignore_item are ignored in the check.
     */
    private boolean enter_through_small_door(MazeListElement p_list_element, Item p_ignore_item) {
        ...
        Set<SearchTreeObject> overlapping_objects = Freerouter.newSortedSet();
    ...
    /**
     * Checks entering a thick room from a via or trace through a small door
     * (after ripup)
     */
