...
public class CalcFromSide {
     ...
     * enters. Used in the push trace algorithm to determine the shove
     * direction. p_no is expected between 1 and p_polyline.line_count - 2
     * inclusive.
     */
    ...
    /**
     * Calculates the nearest border side of p_shape to p_from_point. Used in
     * the shove_drill_item algorithm to determine the shove direction.
     */
    ...
    /**
     * Calculates the Side of p_shape at the start of p_line_segment. If
     * p_shove_to_the_left, the from_side_no is decremented by 2, else it is
     * increased by 2.
     */
    CalcFromSide(LineSegment p_line_segment, TileShape p_shape, boolean p_shove_to_the_left) {
        ...
        FloatPoint prev_corner = p_shape.corner_approx(no);
        FloatPoint next_corner = p_shape.corner_approx((no + 1) % border_line_count);
