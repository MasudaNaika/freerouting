...
public class MenuState extends InteractiveState {
    ...
    public InteractiveState select_items(FloatPoint p_location) {
        hdlg.display_layer_messsage();
        Set<Item> picked_items = hdlg.pick_items(p_location);
        boolean something_found = (picked_items.size() > 0);
        InteractiveState result;
        if (something_found) {
            result = SelectedItemState.get_instance(picked_items, this, hdlg, logfile);
    ...
    public InteractiveState swap_pin(FloatPoint p_location) {
        ...
        if (picked_items.size() > 0) {
            ...
            result = PinSwapState.get_instance(selected_pin, this, hdlg, logfile);
        } else {
    ...
    public InteractiveState key_typed(char p_key_char) {
        ...
        switch (p_key_char) {
            ...
            case 'g':
                ...
                break;
            case 'i':
                curr_return_state = select_items(hdlg.get_current_mouse_position());
                break;
            case 'p':
