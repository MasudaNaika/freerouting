 ...
 * will be sorted in counterclock sense around the border of the shape of
 * p_room. Overlapping neighbours containing an item may be stored in an
 * unordered list.
 *
 * @author Alfons Wirtz
 */
public class SortedRoomNeighbours {
    ...
    public void calculate_new_incomplete_rooms(AutorouteEngine p_autoroute_engine) {
        ...
        Simplex room_simplex = from_room.get_shape().to_Simplex();
        ...
        for (SortedRoomNeighbour next_neighbour : sorted_neighbours) {
            ...
            boolean intersection_with_prev_neighbour_ends_at_corner
                    = (first_touching_side_no != last_touching_side_no || prev_neighbour == sorted_neighbours.last())
            ...
            if (!neighbours_touch) {
                ...
                while (true) {
                    ...
                    if (from_room instanceof IncompleteFreeSpaceExpansionRoom) {
                        IncompleteFreeSpaceExpansionRoom incomplete_room = (IncompleteFreeSpaceExpansionRoom) from_room;
                        if (curr_touching_side_no == last_touching_side_no
                            ...
                            ((CompleteFreeSpaceExpansionRoom) completed_room).set_shape(completed_room.get_shape().intersection(cut_half_plane));
                            corner_cut_off = true;
                    ...
                    boolean last_time
                            ...
                            && !(prev_neighbour == sorted_neighbours.last() && first_time)
                    ...
                    if (!new_room_shape.is_empty()) {
                        ...
                        TileShape new_contained_shape = completed_room.get_shape().intersection(new_room_shape);
                        if (!new_contained_shape.is_empty()) {
                            FreeSpaceExpansionRoom new_room
                                    = p_autoroute_engine.add_incomplete_expansion_room(new_room_shape, from_room.get_layer(), new_contained_shape);
                            ExpansionDoor new_door = new ExpansionDoor(completed_room, new_room, 1);
                            completed_room.add_door(new_door);
                            new_room.add_door(new_door);
                        }
                    }
    ...
    private class SortedRoomNeighbour implements Comparable<SortedRoomNeighbour> {
        ...
        public int compareTo(SortedRoomNeighbour p_other) {
            ...
            if (Math.abs(delta_distance) <= c_dist_tolerance) {
                ...
                if (first_corner().equals(p_other.first_corner())) {
                    ...
                    if (Math.abs(delta_distance) <= c_dist_tolerance) {
                        if (neighbour_room_touch_is_corner && p_other.neighbour_room_touch_is_corner) // Otherwise there may be a short 1 dim. touch at a link between 2 trace lines.
                        ...
                        // the expansion room are identical, so that only 1 obstacle is needed.
                        {
                            ...
                            Line this_compare_line = neighbour_shape.border_line(touching_side_no_of_neighbour_room);
        ...
        /**
         * True, if the intersection of this room and the neighbour is equal to
         * a corner of this room
         */
        public final boolean room_touch_is_corner;

        /**
         * True, if the intersection of this room and the neighbour is equal to
         * a corner of the neighbour room
         */
