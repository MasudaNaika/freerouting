...
public abstract class LocateFoundConnectionAlgo {
    ...
    /**
     * Returns a new Instance of LocateFoundConnectionAlgo or null, if
     * p_destination_door is null.
     */
    ...
    protected LocateFoundConnectionAlgo(MazeSearchAlgo.Result p_maze_search_result, AutorouteControl p_ctrl,
        ...
        backtrack_array = backtrack_list.toArray(BacktrackElement[]::new);
        ...
        start_door = (TargetItemExpansionDoor) start_info.door;
        ...
        while (!connection_done) {
            ...
            if (at_fanout_end) {
                ...
                while (current_target_door_index < backtrack_array.length && !layer_changed) {
                    if (backtrack_array[current_target_door_index].door instanceof ExpansionDrill) {
                        layer_changed = true;
                    } else {
            ...
            if (layer_changed) {
                ...
                ExpansionDrill current_target_drill = (ExpansionDrill) backtrack_array[current_target_door_index].door;
                current_target_shape = TileShape.get_instance(current_target_drill.location);
            } else {
                ...
                if (current_target_shape.dimension() >= 2) {
                    // the target is a conduction area, make a save connection
                    // by shrinking the shape by the trace halfwidth.
                    double trace_half_width = ctrl.compensated_trace_half_width[start_door.room.get_layer()];
                    TileShape shrinked_shape = (TileShape) current_target_shape.offset(-trace_half_width);
            ...
            ResultItem next_trace = calculate_next_trace(layer_changed, at_fanout_end);
            at_fanout_end = false;
    ...
    private ResultItem calculate_next_trace(boolean p_layer_changed, boolean p_at_fanout_end) {
        ...
        if (!p_at_fanout_end) {
            FloatPoint adjusted_start_corner = adjust_start_corner();
            if (adjusted_start_corner != current_from_point) {
                FloatPoint add_corner = calculate_additional_corner(current_from_point, adjusted_start_corner,
                        true, angle_restriction);
        ...
        FloatPoint prev_corner = current_from_point;
        while (true) {
        ...
        if (p_layer_changed) {
            ...
            CompleteExpansionRoom next_room = backtrack_array[current_from_door_index].next_room;
            if (next_room != null) {
        ...
        for (FloatPoint fp : corner_list) {
            IntPoint curr_point = fp.round();
        ...
        IntPoint[] corner_arr = rounded_corner_list.toArray(IntPoint[]::new);
        ResultItem result = new ResultItem(corner_arr, current_trace_layer);
        current_trace_layer = next_layer;
        return result;
    }

    /**
     * Returns the next list of corners for the construction of the trace in
    ...
    /**
     * Calculates the starting point of the next trace on p_from_door.item. The
    ...
    /**
     * Creates a list of doors by backtracking from p_destination_door to the
     * start door. Returns null, if p_destination_door is null.
     */
    ...
    /**
     * Adjusts the start corner, so that a trace starting at this corner is
     * completely contained in the start room.
     */
    private FloatPoint adjust_start_corner() {
        if (current_from_door_index < 0) {
            return current_from_point;
        }
        BacktrackElement curr_from_info = backtrack_array[current_from_door_index];
        if (curr_from_info.next_room == null) {
            return current_from_point;
        }
        double trace_half_width = ctrl.compensated_trace_half_width[current_trace_layer];
    ...
    /**
     * Calculates an additional corner, so that for the lines from p_from_point
     * to the result corner and from the result corner to p_to_point
     * p_angle_restriction is fulfilled.
     */
    ...
    /**
     * Type of a single item in the result list connection_items. Used to create
     * a new PolylineTrace.
     */
    ...
    /**
     * Type of the elements of the list returned by this.backtrack(). Next_room
     * is the common room of the current door and the next door in the backtrack
     * list.
     */
