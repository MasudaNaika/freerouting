...
/**
 * Class with functions for checking and inserting pads with eventually shoving
 * aside obstacle traces.
 *
 * @author Alfons Wirtz
 */
public class ForcedPadAlgo {
    ...
    /**
     * Checks, if possible obstacle traces can be shoved aside, so that a pad
     * with the input parameters can be inserted without clearance violations.
     * Returns false, if the check failed. If p_ignore_items != null, items in
     * this list are not checked, If p_check_only_front only trace obstacles in
     * the direction from p_from_side are checked for performance reasons. This
     * is the cave when moving drill_items
     */
     ...
     * failed. In this case the database may be damaged, so that an undo becomes
     * necessesary.
     */
    boolean forced_pad(TileShape p_pad_shape, CalcFromSide p_from_side,
        ...
        if (!MoveDrillItemAlgo.shove_vias(p_pad_shape, p_from_side, p_layer, p_net_no_arr, p_cl_type,
                p_ignore_items, p_max_recursion_depth, p_max_via_recursion_depth, false, board)) {
            return false;
        }
        ShapeSearchTree search_tree = board.search_tree_manager.get_default_tree();
        ShapeTraceEntries shape_entries
