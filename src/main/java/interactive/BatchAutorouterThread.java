...
public class BatchAutorouterThread extends InteractiveActionThread {
    ...
    protected void thread_action() {
        ...
        try {
            ...
            if (hdlg.get_settings().autoroute_settings.get_with_autoroute() && !is_stop_requested()) {
            ...
            if (hdlg.get_settings().autoroute_settings.get_with_postroute() && !is_stop_requested()) {
                ...
                batch_opt_route.optimize_board();
                String curr_message;
...
//            Freerouter.logError(e);
    ...
    public void draw(Graphics p_graphics) {
        ...
        if (curr_air_line != null) {
            ...
            hdlg.graphics_context.draw(draw_line, draw_width, draw_color, p_graphics, 1);
        }
        FloatPoint current_opt_position = batch_opt_route.get_current_position();
        int radius = 10 * hdlg.get_routing_board().rules.get_default_trace_half_width(0);
        if (current_opt_position != null) {
            ...
            hdlg.graphics_context.draw(draw_points, draw_width, draw_color, p_graphics, 1);
            draw_points[0] = new FloatPoint(current_opt_position.x + radius, current_opt_position.y - radius);
            draw_points[1] = new FloatPoint(current_opt_position.x - radius, current_opt_position.y + radius);
            hdlg.graphics_context.draw(draw_points, draw_width, draw_color, p_graphics, 1);
            hdlg.graphics_context.draw_circle(current_opt_position, radius, draw_width, draw_color, p_graphics, 1);
        }
    }
