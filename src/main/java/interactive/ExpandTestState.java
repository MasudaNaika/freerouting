...
public class ExpandTestState extends InteractiveState {
        ...
        if (p_key_char == 'n') {
            if (in_autoroute) {
                ...
                while (!completing_succeeded) {
                    IncompleteFreeSpaceExpansionRoom next_room = autoroute_engine.get_first_incomplete_expansion_room();
                    if (next_room == null) {
                        hdlg.screen_messages.set_status_message("expansion completed");
                        break;
                    }
    ...
    public void draw(Graphics p_graphics) {
        ...
        if (autoroute_result != null) {
            autoroute_result.draw(p_graphics, hdlg.graphics_context);
        }
    }
    ...
    private void init(FloatPoint p_location) {
        ...
        control_settings = new AutorouteControl(hdlg.get_routing_board(), route_net_no, hdlg.settings);
        // control_settings.ripup_allowed = true;
        // control_settings.is_fanout = true;
        control_settings.ripup_pass_no = hdlg.settings.autoroute_settings.get_pass_no();
        control_settings.ripup_costs = control_settings.ripup_pass_no * hdlg.settings.autoroute_settings.get_start_ripup_costs();
        control_settings.vias_allowed = false;
        autoroute_engine = new AutorouteEngine(board, control_settings.trace_clearance_class_no, false);
        autoroute_engine.init_connection(route_net_no, null, null);
        if (route_item == null) {
        ...
        if (route_dest_set.size() > 0) {
            ...
            in_autoroute = (maze_search_algo != null);
        }
    }
