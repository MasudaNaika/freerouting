...
public class ItemColorTableModel extends ColorTableModel implements Serializable {
    ...
    public ItemColorTableModel(board.LayerStructure p_layer_structure, Locale p_locale) {
    ...
    private void set_colors(int p_item_type, Color[] p_color_arr) {
        for (int layer = 0; layer < data.length - 1; ++layer) {
            ...
            data[layer][p_item_type] = p_color_arr[color_index];
        }
        data[data.length - 1][p_item_type] = p_color_arr[p_color_arr.length - 1];
        item_colors_precalculated = false;
    }
