...
public class ObstacleExpansionRoom implements CompleteExpansionRoom {
    ...
    ObstacleExpansionRoom(Item p_item, int p_index_in_item, ShapeSearchTree p_shape_tree) {
        ...
        shape = p_item.get_tree_shape(p_shape_tree, p_index_in_item);
    ...
    /**
     * Creates a 2-dim door with the other obstacle room, if that is useful for
     * the autoroute algorithm. It is assumed that this room and p_other have a
    ...
    public boolean create_overlap_door(ObstacleExpansionRoom p_other) {
        if (door_exists(p_other)) {
            return false;
        }
        if (!(item.is_route() && p_other.item.is_route())) {
            return false;
        }
        if (!item.shares_net(p_other.item)) {
            return false;
        }
    ...
    public void reset_doors() {
        for (ExpandableObject curr_door : doors) {
            curr_door.reset();
        }
    }
