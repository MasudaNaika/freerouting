 ...
 * @author alfons
 */
public class Padstack implements Comparable<Padstack>, board.ObjectInfoPanel.Printable, Serializable {

    /**
     * Creates a new Padstack with shape p_shapes[i] on layer i (0 <= i <
     * p_shapes.length). p_is_drilllable indicates, if vias of the own net are
     * allowed to overlap with this padstack If p_placed_absolute is false, the
     * layers of the padstack are mirrored, if it is placed on the back side.
    ...
    /**
     * Compares 2 padstacks by name. Useful for example to display padstacks in
     * alphabetic order.
     */
    @Override
     ...
     * padstack on layer p_layer. If the length of the pad is smaller than
     * p_factor times the height of the pad, connection also to the long side is
     * allowed.
     */
