...
public class ShapeSearchTree extends datastructures.MinAreaTree {
     ...
     * clearance class number for which the shapes of this tree is compensated.
    ...
    /**
     * Returns, if for the shapes stored in this tree clearance compensatiion is
     * used.
     */
    ...
    public int clearance_compensation_value(int p_clearance_class_no, int p_layer) {
        ...
        int result = board.rules.clearance_matrix.value(p_clearance_class_no, compensated_clearance_class_no, p_layer) - board.rules.clearance_matrix.clearance_compensation_value(compensated_clearance_class_no, p_layer);
        return Math.max(result, 0);
    }

    /**
     * Changes the tree entries from p_keep_at_start_count + 1 to
     * new_shape_count - 1 - keep_at_end_count to p_changed_entries. Special
     * implementation for change_trace for performance reasons
     */
    void change_entries(PolylineTrace p_obj, Polyline p_new_polyline,
        ...
        int compensated_half_width = p_obj.get_half_width()
                + clearance_compensation_value(p_obj.clearance_class_no(), p_obj.get_layer());
        TileShape[] changed_shapes
                = offset_shapes(p_new_polyline, compensated_half_width, p_keep_at_start_count,
    ...
    void merge_entries_in_front(PolylineTrace p_from_trace, PolylineTrace p_to_trace,
        ...
        int compensated_half_width = p_to_trace.get_half_width()
                + clearance_compensation_value(p_to_trace.clearance_class_no(), p_to_trace.get_layer());
        TileShape[] link_shapes
                = offset_shapes(p_joined_polyline, compensated_half_width, p_from_entry_no, p_to_entry_no);
     ...
     * after a moddle piece was cut out. Special implementation for
     * ShapeTraceEntries.fast_cutout_trace for performance reasoms.
     */
     ...
     * p_obstacles. If p_layer < 0, the layer is ignored.
     */
    ...
    /**
     * Returns all SearchTreeObjects on layer p_layer, which overlap with
     * p_shape. If p_layer < 0, the layer is ignored
     */
    public Set<SearchTreeObject> overlapping_objects(ConvexShape p_shape, int p_layer) {
        Set<SearchTreeObject> result = Freerouter.newSortedSet();
        overlapping_objects(p_shape, p_layer, new int[0], result);
        return result;
    }

    /**
     * Puts all tree entries overlapping with p_shape on layer p_layer into the
     * list p_obstacles. If p_layer < 0, the layer is ignored.
     */
    ...
    /**
     * Puts all tree entries overlapping with p_shape on layer p_layer into the
     * list p_obstacles. If p_layer < 0, the layer is ignored. tree_entries with
     * object containing a net number of p_ignore_net_nos are ignored.
     */
    public void overlapping_tree_entries(ConvexShape p_shape, int p_layer, int[] p_ignore_net_nos,
        ...
        for (Leaf curr_leaf : tmp_list) {
            ...
            if (!ignore_object) {
                ...
                boolean add_item;
                if (is_45_degree && curr_shape instanceof IntOctagon) // in this case the check for intersection is redundant and
     ...
     * shape p_shape, net number p_net_no, clearance type p_cl_type and layer
     * p_layer whould produce a clearance violation, and puts them into the set
     * p_obstacle_entries. The elements in p_obstacle_entries are of type
     * TreeEntry. if p_layer < 0, the layer is ignored. Used only internally,
     * because the clearance compensation is not taken iinnto account.
     */
    void overlapping_tree_entries_with_clearance(ConvexShape p_shape, int p_layer, int[] p_ignore_net_nos,
        ...
        Set<EntrySortedByClearance> sorted_items = Freerouter.newSortedSet();
    ...
    /**
     * Puts all items in the tree overlapping with p_shape on layer p_layer into
     * p_obstacles, if p_obstacles != null. If p_layer < 0, the layer is
     * ignored.
     */
     ...
     * describes the required clearance restrictions to other items. The
     * function may also return items, which are nearly overlapping, but do not
     * overlap with exact calculation. If p_layer < 0, the layer is ignored.
     */
    public Set<Item> overlapping_items_with_clearance(ConvexShape p_shape, int p_layer, int[] p_ignore_net_nos,
        ...
        Set<SearchTreeObject> overlaps = Freerouter.newSortedSet();
    ...
    /**
     * Returns all objects of class TreeEntry, which overlap with p_shape on
     * layer p_layer inclusive clearance. p_clearance_class is the index in the
     * clearance matrix, which describes the required clearance restrictions to
     * other items. If p_layer < 0, the layer is ignored.
     */
    public Collection<TreeEntry> overlapping_tree_entries_with_clearance(ConvexShape p_shape, int p_layer,
        ...
        if (is_clearance_compensation_used()) {
            overlapping_tree_entries(p_shape, p_layer, p_ignore_net_nos, result);
        } else {
            overlapping_tree_entries_with_clearance(p_shape, p_layer, p_ignore_net_nos,
    ...
    /**
     * Calculates a new incomplete room with a maximal TileShape contained in
     * the shape of p_room, which may overlap only with items of the input net
     * on the input layer. p_room.get_contained_shape() will be contained in the
     * shape of the result room. If that is not possible, several rooms are
     * returned with shapes, which intersect with p_room.get_contained_shape().
     * The result room is not yet complete, because its doors are not yet
     * calculated. If p_ignore_shape != null, objects of type
     * CompleteFreeSpaceExpansionRoom, whose intersection with the shape of
     * p_room is containes in p_ignore_shape, are ignored.
     */
    public Collection<IncompleteFreeSpaceExpansionRoom> complete_shape(IncompleteFreeSpaceExpansionRoom p_room, int p_net_no,
        ...
        RegularTileShape bounding_shape = start_shape.bounding_shape(bounding_directions);
     ...
     * intersect with the interiour of p_obstacle_shape.
    ...
    TileShape[] calculate_tree_shapes(ObstacleArea p_obstacle_area) {
        ...
        if (board.communication.host_cad_exists()) {
            max_tree_shape_width = Math.min(500 * board.communication.get_resolution(Unit.MIL), max_tree_shape_width);
    ...
    TileShape[] calculate_tree_shapes(BoardOutline p_board_outline) {
        ...
        if (p_board_outline.keepout_outside_outline_generated()) {
            ...
            Collection<TileShape> tree_shape_list = new LinkedList<>();
            for (int layer_no = 0; layer_no < board.layer_structure.arr.length; ++layer_no) {
                for (int i = 0; i < convex_shapes.length; ++i) {
                    TileShape curr_convex_shape = convex_shapes[i];
                    int offset_width = clearance_compensation_value(p_board_outline.clearance_class_no(), 0);
                    curr_convex_shape = (TileShape) curr_convex_shape.enlarge(offset_width);
                    tree_shape_list.add(curr_convex_shape);
                }
            }
            result = tree_shape_list.toArray(TileShape[]::new);
        } else {
            // Only the line shapes of the outline are inserted as obstales into the tree.
            result = new TileShape[p_board_outline.line_count() * board.layer_structure.arr.length];
            ...
            for (int layer_no = 0; layer_no < board.layer_structure.arr.length; ++layer_no) {
                for (int shape_no = 0; shape_no < p_board_outline.shape_count(); ++shape_no) {
                    ...
                    for (int i = 0; i < border_line_count; ++i) {
                        ...
                        int cmp_value = clearance_compensation_value(p_board_outline.clearance_class_no(), 0);
    ...
    TileShape[] calculate_tree_shapes(PolylineTrace p_trace) {
        ...
        int offset_width = p_trace.get_half_width()
                + clearance_compensation_value(p_trace.clearance_class_no(), p_trace.get_layer());
    ...
    /**
     * Makes shure that on each layer there will be more than 1
     * IncompleteFreeSpaceExpansionRoom, even if there are no objects on the
     * layer. Otherwise the maze search algprithm gets problems with vias.
     */
