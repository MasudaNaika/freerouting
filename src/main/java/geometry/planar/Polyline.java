 ...
 * of consecutive lines. The lines of the objects of class Polyline are normally
 * defined by points with integer coordinates, wheras the intersections of Lines
 * can be representated in general only by infinite precision rational points.
 * We use polylines with integer coordinates instead of polygons with infinite
 * precision rational coordinates because of its better performance in geometric
 * calculations.
 *
 *
 * @author Alfons Wirtz
...
public class Polyline implements Serializable {
     ...
     * so that the i-th corner of p_polygon will be the intersection of the i-th
     * and the i+1-th lines of the new created p_polyline for 0 <= i <
     * p_point_arr.length. p_polygon must have at least 2 corners
     */
    ...
    /**
     * Creates a polyline from an array of lines. Lines, which are parallel to
     * the previous line are skipped. The directed lines are normalized, so that
     * they intersect the previous line before the next line
     */
    ...
    public boolean is_point() {
        ...
        Point first_corner = corner(0);
        for (int i = 1; i < arr.length - 1; ++i) {
            if (!(corner(i).equals(first_corner))) {
                return false;
            }
        }
        return true;
    ...
    /**
     * returns the intersection of the last line with the line before the last
     * line
     */
    ...
    public Point[] corner_arr() {
        ...
        if (precalculated_corners == null) // corner array is not yet allocated
        {
    ...
    public FloatPoint[] corner_approx_arr() {
        ...
        if (precalculated_float_corners == null) // corner array is not yet allocated
        {
    ...
    /**
     * Returns an approximation of the intersection of the p_no-th with the
     * (p_no - 1)-th line by a FloatPoint.
     */
    public FloatPoint corner_approx(int p_no) {
        ...
        if (precalculated_float_corners[no] == null) // corner is not yet calculated
        {
    ...
    public Point corner(int p_no) {
        ...
        if (precalculated_corners[no] == null) // corner is not yet calculated
        {
    ...
    /**
     * calculates for each line a shape around this line where the right and
     * left edge lines have the distance p_half_width from the center line
     ...
     * p_half_width from the center line
     */
    public TileShape[] offset_shapes(int p_half_width,
            ...
            for (int j = i + 2; j < arr.length - 1; ++j) {
                ...
                if (!direction_changed) {
                    ...
                    if (next_border_line.side_of(corner_to_check) == Side.ON_THE_LEFT
                            && next_border_line.side_of(corner(i)) == Side.ON_THE_RIGHT
                            && next_border_line.side_of(corner(i - 1)) == Side.ON_THE_RIGHT) // an outstanding corner
                    {
            ...
            for (int j = i - 2; j >= 1; --j) {
                ...
                if (!direction_changed) {
                    ...
                    if (prev_border_line.side_of(corner_to_check) == Side.ON_THE_LEFT
                            && prev_border_line.side_of(corner(i)) == Side.ON_THE_RIGHT
                            && prev_border_line.side_of(corner(i - 1)) == Side.ON_THE_RIGHT) // an outstanding corner
                    {
    ...
    /**
     * Calculates for the p_no-th line segment a shape around this line where
     * the right and left edge lines have the distance p_half_width from the
     * center line. 0 <= p_no <= arr.length - 3
     */
     ...
     * where the border lines have the distance p_half_width from the center
    ...
    /**
     * Returns the smallest box containing the intersection points of the lines
     * of this polyline
     */
     ...
     * index p_from_corner_no to index p_to_corner_no of the lines of this
     * polyline
     */
    ...
    /**
     * Calculates an aproximation of the nearest point on this polyline to
     * p_from_point.
     */
    ...
    /**
     * Calculates the distance of p_from_point to the the nearest point on this
     * polyline
     */
    ...
    /**
     * Combines the two polylines, if they have a common end corner. The order
     * of lines in this polyline will be preserved. Returns the combined
     * polyline or this polyline, if this polyline and p_other have no common
     * end corner. If there is something to combine at the start of this
     * polyline, p_other is inserted in front of this polyline. If there is
     * something to combine at the end of this polyline, this polyline is
     * inserted in front of p_other.
     */
    ...
    /**
     * Splits this polyline at the line with number p_line_no into two by
     * inserting p_endline as concluding line of the first split piece and as
     * the start line of the second split piece. p_endline and the line with
     * number p_line_no must not be parallel. The order of the lines ins the two
     * result pieces is preserved. p_line_no must be bigger than 0 and less then
     * arr.length - 1. Returns null, if nothing was split.
     */
    public Polyline[] split(int p_line_no, Line p_end_line) {
        ...
        Point new_end_corner = arr[p_line_no].intersection(p_end_line);
        if (p_line_no <= 1 && new_end_corner.equals(first_corner())
                || p_line_no >= arr.length - 2 && new_end_corner.equals(last_corner())) {
        ...
        if (corner(p_line_no).equals(new_end_corner)) {
            ...
            System.arraycopy(arr, p_line_no, second_piece, 0, second_piece.length);
    ...
    /**
     * create a new Polyline by skipping the lines of this Polyline from
     * p_from_no to p_to_no
     */
     ...
     * line segment of this polyline to p_from_side. Returns null, if the
     * perpendicular line does not intersect the neares line segment inside its
     * segment bounds or if p_from_point is contained in this polyline.
     */
    public LineSegment projection_line(Point p_from_point) {
        ...
        for (int i = 1; i < arr.length - 1; ++i) {
            ...
            if (curr_distance < min_distance) {
                Direction direction_towards_line = arr[i].perpendicular_direction(p_from_point);
                if (direction_towards_line == null) {
                    continue;
                }
                Line curr_result_line = new Line(p_from_point, direction_towards_line);
                Point prev_corner = corner(i - 1);
                Point next_corner = corner(i);
    ...
    /**
     * Shortens this polyline to p_new_line_count lines. Additioanally the last
     * line segment will be approximately shortened to p_new_length. The last
     * corner of the new polyline will be an IntPoint.
     */
    public Polyline shorten(int p_new_line_count, double p_last_segment_length) {
        FloatPoint last_corner = corner_approx(p_new_line_count - 2);
        FloatPoint prev_last_corner = corner_approx(p_new_line_count - 3);
