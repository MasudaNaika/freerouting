...
public class GraphicsContext implements Serializable {
    ...
    public GraphicsContext(IntBox p_design_bounds,
            Dimension p_panel_bounds, board.LayerStructure p_layer_structure, Locale p_locale) {
    ...
    public GraphicsContext(GraphicsContext p_graphics_context) {
        coordinate_transform = new CoordinateTransform(p_graphics_context.coordinate_transform);
    ...
    /**
     * Changes the bounds of the board design to p_design_bounds. Useful when
     * components are still placed outside the boaed.
     */
    public void change_design_bounds(IntBox p_new_design_bounds) {
        ...
        coordinate_transform = new CoordinateTransform(p_new_design_bounds, screen_bounds);
    }
    ...
    public void draw_boundary(Shape p_shape, double p_draw_half_width, Color p_color, Graphics p_g, double p_translucency_factor) {
        if (p_shape instanceof PolylineShape) {
            ...
            draw_circle(curr_circle.center.to_float(), curr_circle.radius, p_draw_half_width,
    ...
    /**
     * Fill the interiour of a list of polygons. Used for example with an area
     * consisting of a border polygon and some holes.
     */
    ...
    /**
     * Sets the factor for automatic layer dimming. Values are between 0 and 1.
     * If 1, there is no automatic layer dimming.
     */
    ...
    /**
     * Gets the visibility factor of the input layer. The result is between 0
     * and 1. If the result is 0, the layer is invisible, if the result is 1,
     * the layer is fully visible.
     */
    ...
    /**
     * Gets the visibility factor of the input layer without the aoutomatic
     * layer dimming.
     */
    ...
    /**
     * Gets the visibility factor of the input layer. The value is expected
     * between 0 and 1. If the value is 0, the layer is invisible, if the value
     * is 1, the layer is fully visible.
     *
     */
     ...
     * performance of the draw function by avoiding unnessesary calls of draw
     * (line)
     */
