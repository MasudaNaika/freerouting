...
/**
 *
 * Auxiliary class containing internal functions for pulling any angle traces
 * tight.
 *
 *
 * @author Alfons Wirtz
...
class PullTightAlgoAnyAngle extends PullTightAlgo {
        ...
        for (int i = 0; i <= last_index; ++i) {
            ...
            if (in_clip_shape) {
                ...
                if (ok) {
                    ...
                    // by twice rounding.
                    if (length_after >= length_before) // May happen from rounding to integer.
    ...
    /**
     * tries to reduce te number of lines of p_polyline by moving lines parallel
     * beyond the intersection of the next or privious lines.
     */
    private Polyline reduce_lines(Polyline p_polyline) {
        ...
        for (int i = 2; i < line_arr.length - 2; ++i) {
            ...
            if (Signum.of(prev_dist) != Signum.of(next_dist)) // the 2 corners are on different sides of the translate_line
            {
                continue;
            }
    ...
    private Line smoothen_corner(Line[] p_line_arr, int p_start_no) {
        ...
        while (Math.abs(delta_dist) > min_translate_dist) {
            ...
            } else // moved a little bit to far at the first time
    ...
    protected Line reposition_line(Line[] p_line_arr, int p_start_no) {
        ...
        while (Math.abs(prev_dist) < c_epsilon) // move also all lines trough the start corner of the line to translate
        {
            ++corners_skipped_before;
            int curr_no = p_start_no - corners_skipped_before;
            if (curr_no < 0) // the first corner is on the line to translate
            {
                return null;
            }
        ...
        double next_dist = translate_line.signed_distance(next_corner);
        while (Math.abs(next_dist) < c_epsilon) // move also all lines trough the end corner of the line to translate
        {
            ++corners_skipped_after;
            int curr_no = p_start_no + 3 + corners_skipped_after;
            if (curr_no >= p_line_arr.length - 2) // the last corner is on the line to translate
            {
                return null;
            }
        ...
        if (Signum.of(prev_dist) != Signum.of(next_dist)) // the 2 corners are at different sides of translate_line
        {
            return null;
        }
        FloatPoint nearest_point;
    ...
    private Polyline skip_lines(Polyline p_polyline) {
        for (int i = 1; i < p_polyline.arr.length - 3; ++i) {
            for (int j = 0; j <= 1; ++j) {
                ...
                if (side1 != side2) // the two corners are on different sides of the line
                {
                ...
                if (j == 0) // curr_line is 1 line later than in the case skipping 1 line
    ...
    Polyline smoothen_start_corner_at_trace(PolylineTrace p_trace) {
        ...
        if (acute_angle) {
            ...
            double translate_dist = (Limits.sqrt2 - 1) * curr_half_width;
