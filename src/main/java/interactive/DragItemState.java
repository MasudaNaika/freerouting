...
public class DragItemState extends DragState {
    ...
    /**
     * Moves the items of the group to p_to_location. Return this.return_state,
     * if an error eccured while moving, so that an undo may be necessary.
     */
    @Override
    public InteractiveState move_to(FloatPoint p_to_location) {
        IntPoint to_location = p_to_location.round();
        IntPoint from_location = previous_location.round();
        ...
        if (shove_ok) {
            if (!something_dragged) {
                ...
                if (logfile != null) {
                    ...
                    logfile.start_scope(LogfileScope.DRAGGING_ITEMS, previous_location);
                }
        ...
        previous_location = p_to_location;//(IntPoint)curr_location.translate_by(rel_coor);
        return this;
    }
