...
public class RoutingBoard extends BasicBoard implements Serializable {
     ...
     * items. Among other things it may contain a clearance matrix.
     */
    ...
    public void additional_update_after_change(Item p_item) {
        ...
        if (autoroute_engine == null || !autoroute_engine.maintain_database) {
            return;
        }
        // Invalidate the free space expansion rooms touching a shape of p_item.
        int shape_count = p_item.tree_shape_count(autoroute_engine.autoroute_search_tree);
        for (int i = 0; i < shape_count; ++i) {
            TileShape curr_shape = p_item.get_tree_shape(autoroute_engine.autoroute_search_tree, i);
            autoroute_engine.invalidate_drill_pages(curr_shape);
            int curr_layer = p_item.shape_layer(i);
            Collection<SearchTreeObject> overlaps
                    = autoroute_engine.autoroute_search_tree.overlapping_objects(curr_shape, curr_layer);
            for (SearchTreeObject curr_object : overlaps) {
                if (curr_object instanceof CompleteFreeSpaceExpansionRoom) {
                    autoroute_engine.remove_complete_expansion_room((CompleteFreeSpaceExpansionRoom) curr_object);
                }
            }
        }
     ...
     * Returns false, if some items could not be removed, because they were
     * fixed.
     */
    ...
    /**
     * Optimizes the route in the internally marked area. If p_net_no > 0, only
     * traces with net number p_net_no are optimized. If p_clip_shape != null
     * the optimizing is restricted to p_clip_shape. p_trace_cost_arr is used
     * for optimizing vias and may be null. If p_stoppable_thread != null, the
     * agorithm can be requested to be stopped. If p_time_limit > 0; the
     ...
     * for optimizing vias and may be null. If p_stoppable_thread != null, the
     * agorithm can be requested to be stopped. If p_time_limit > 0; the
     * algorithm will be stopped after p_time_limit Milliseconds. If
     * p_keep_point != null, traces on layer p_keep_point_layer containing
     * p_keep_point will also contain this point after optimizing.
     */
    public void opt_changed_area(int[] p_only_net_no_arr, IntOctagon p_clip_shape, int p_accuracy, ExpansionCostFactor[] p_trace_cost_arr,
    ...
    /**
     * Checks if a rectangular boxed trace line segment with the input
     * parameters can be inserted without conflict. If a conflict exists, The
     * result length is the maximal line length from p_line.a to p_line.b, which
     * can be inserted without conflict (Integer.MAX_VALUE, if no conflict
     ...
     * without conflict. If a conflict exists, The result length is the maximal
     * line length from p_line.a to p_line.b, which can be inserted without
     * conflict (Integer.MAX_VALUE, if no conflict exists). If
    ...
    public double check_trace_segment(LineSegment p_line_segment, int p_layer, int[] p_net_no_arr,
        ...
        for (TreeEntry curr_obstacle_entry : obstacle_entries) {
            ...
            if (default_tree.is_clearance_compensation_used()) {
                ...
                int clearance_value = clearance_value(curr_obstacle.clearance_class_no(), p_cl_class_no, p_layer);
    ...
    /**
     * Checks, if p_item can be translated by p_vector without producing
    ...
    public boolean check_move_item(Item p_item, Vector p_vector, Collection<Item> p_ignore_items) {
        ...
        for (int i = 0; i < p_item.tile_shape_count(); ++i) {
            ...
            Set<Item> obstacles
                    = overlapping_items_with_clearance(moved_shape, p_item.shape_layer(i), p_item.net_no_arr,
     ...
     * Returns false, if that was not possible without creating clearance
     * violations. In this case the database may be damaged, so that an undo
     * becomes necessesary.
     */
     ...
     * ignored. Returns null, if no item is found, If p_layer < 0, the layer is
     * ignored
     */
     ...
     * inserted without clearance violations. If the shove failed, the database
     * may be damaged, so that an undo becomes necessesary. Returns false, if
    ...
    public boolean forced_via(ViaInfo p_via_info, Point p_location, int[] p_net_no_arr,
        ...
        if (result) {
            ...
            opt_changed_area(opt_net_no_arr, tidy_clip_shape,
    ...
    /**
     * Tries to insert a trace line with the input parameters from p_from_corner
     * to p_to_corner while shoving aside obstacle traces and vias. Returns the
     * last point between p_from_corner and p_to_corner, to which the shove
     * succeeded. Returns null, if the check was inaccurate and an error accured
     * while inserting, so that the database may be damaged and an undo
     * necessary. p_search_tree is the shape search tree used in the algorithm.
     */
    ...
    /**
     * Tries to insert a trace polyline with the input parameters from while
     * shoving aside obstacle traces and vias. Returns the last corner on the
     * polyline, to which the shove succeeded. Returns null, if the check was
     * inaccurate and an error accured while inserting, so that the database may
     * be damaged and an undo necessary.
     */
    public Point insert_forced_trace_polyline(Polyline p_polyline, int p_half_width, int p_layer, int[] p_net_no_arr,
        ...
        Set<Item> picked_items = pick_items(from_corner, p_layer, filter);
        ...
        if (new_trace.normalize(changed_area.get_area(p_layer))) {
            ...
            Set<Item> curr_picked_items = pick_items(new_corner, p_layer, item_filter);
            new_trace = null;
     ...
     * p_retain_autoroute_database, the autoroute database is retained and
    ...
    public AutorouteEngine init_autoroute(int p_net_no, int p_trace_clearance_class_no,
        ...
        if (autoroute_engine == null || !p_retain_autoroute_database || autoroute_engine.autoroute_search_tree.compensated_clearance_class_no != p_trace_clearance_class_no) {
            autoroute_engine = new AutorouteEngine(this, p_trace_clearance_class_no, p_retain_autoroute_database);
        }
        autoroute_engine.init_connection(p_net_no, p_stoppable_thread, p_time_limit);
    ...
    public void finish_autoroute() {
        ...
        autoroute_engine = null;
    }

    /**
     * Routes automatically p_item to another item of the same net, to which it
     * is not yet electrically connected. Returns an enum of type
    ...
    /**
     * Autoroutes from the input pin until the first via, in case the pin and
     * its connected set has only 1 layer. Ripup is allowed if p_ripup_costs is
     * >= 0. Returns an enum of type AutorouteEngine.AutorouteResult
     */
    ...
    public boolean connect_to_trace(IntPoint p_from_point, Trace p_to_trace,
        ...
        if (!check_polyline_trace(connection_line, trace_layer, p_pen_half_width,
                p_to_trace.net_no_arr, p_cl_type)) {
            return false;
        }
        if (changed_area != null) {
            for (int i = 0; i < connection_line.corner_count(); ++i) {
                changed_area.join(connection_line.corner_approx(i), trace_layer);
            }
        }

        insert_trace(connection_line, trace_layer, p_pen_half_width, net_no_arr, p_cl_type, FixedState.UNFIXED);
        if (!p_from_point.equals(first_corner)) {
            Trace tail = get_trace_tail(first_corner, trace_layer, net_no_arr);
            if (tail != null && !tail.is_user_fixed()) {
                remove_item(tail);
            }
        }
        if (!p_from_point.equals(last_corner)) {
            Trace tail = get_trace_tail(last_corner, trace_layer, net_no_arr);
            if (tail != null && !tail.is_user_fixed()) {
                remove_item(tail);
            }
        }
    ...
    /**
     * Checks, if the list p_items contains traces, which have no contact at
     * their start or end point. Trace with net number p_except_net_no are
     * ignored.
     */
    public boolean contains_trace_tails(Collection<Item> p_items, int[] p_except_net_no_arr) {
        for (Item curr_ob : p_items) {
            if (curr_ob instanceof Trace) {
                ...
                if (!curr_trace.nets_equal(p_except_net_no_arr) && curr_trace.is_tail()) {
                    return true;
                }
            }
        }
    ...
    /**
     * Removes all trace tails of the input net. If p_net_no <= 0, the tails of
    ...
    public boolean remove_trace_tails(int p_net_no, Item.StopConnectionOption p_stop_connection_option) {
        ...
        remove_items(stub_connections, false);
    ...
    /**
     * Sets, if all conduction areas on the board are obstacles for route of
     * foreign nets.
     */
    public void change_conduction_is_obstacle(boolean p_value) {
        if (rules.get_ignore_conduction() != p_value) {
            return; // no muultiply
        }
        ...
        rules.set_ignore_conduction(!p_value);
    ...
    /**
     * Tries to educe the nets of traces and vias, so that the nets are a subset
     * of the nets of the contact items. This is applied to traces and vias with
     * more than 1 net connected to tie pins. Returns true, if the nets of some
     * items were reduced.
     */
