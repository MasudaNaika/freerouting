...
public class WindowObjectVisibility extends WindowVisibility {
    ...
    public static WindowObjectVisibility get_instance(BoardFrame p_board_frame) {
        ResourceBundle resources
                = ResourceBundle.getBundle("gui.resources.WindowObjectVisibility", p_board_frame.get_locale());
    ...
    @Override
    public void refresh() {
        boardgraphics.ColorIntensityTable color_intensity_table = get_board_handling().graphics_context.color_intensity_table;
        for (int i = 0; i < ObjectNames.values().length; ++i) {
            set_slider_value(i, color_intensity_table.get_value(i));
        }
    }
