...
public class GUIDefaultsFile {
    ...
    public static boolean write(gui.BoardFrame p_board_frame,
            interactive.BoardHandling p_board_handling, OutputStream p_output_stream) {
    ...
    public static boolean read(gui.BoardFrame p_board_frame,
            interactive.BoardHandling p_board_handling, InputStream p_input_stream) {
    ...
    private void write_windows_scope() throws IOException {
        ...
        write_frame_scope(board_frame.route_parameter_window.detail_window, "route_details");
    ...
    private boolean read_frame_scope(Keyword p_frame) throws IOException {
        ...
        if (null == p_frame) {
            ...
            switch (p_frame) {
                case BOARD_FRAME:
                    curr_frame = board_frame;
                    break;
                case COLOR_MANAGER:
                    curr_frame = board_frame.color_manager;
                    break;
                case OBJECT_VISIBILITY:
                    curr_frame = board_frame.object_visibility_window;
                    break;
                case LAYER_VISIBILITY:
                    curr_frame = board_frame.layer_visibility_window;
                    break;
                case DISPLAY_MISCELLANIOUS:
                    curr_frame = board_frame.display_misc_window;
                    break;
                case SNAPSHOTS:
                    curr_frame = board_frame.snapshot_window;
                    break;
                case SELECT_PARAMETER:
                    curr_frame = board_frame.select_parameter_window;
                    break;
                case ROUTE_PARAMETER:
                    curr_frame = board_frame.route_parameter_window;
                    break;
                case MANUAL_RULES:
                    curr_frame = board_frame.route_parameter_window.manual_rule_window;
                    break;
                case ROUTE_DETAILS:
                    curr_frame = board_frame.route_parameter_window.detail_window;
                    break;
                case MOVE_PARAMETER:
                    curr_frame = board_frame.move_parameter_window;
                    break;
                case CLEARANCE_MATRIX:
                    curr_frame = board_frame.clearance_matrix_window;
                    break;
                case VIA_RULES:
                    curr_frame = board_frame.via_window;
                    break;
                case EDIT_VIAS:
                    curr_frame = board_frame.edit_vias_window;
                    break;
                case EDIT_NET_RULES:
                    curr_frame = board_frame.edit_net_rules_window;
                    break;
                case ASSIGN_NET_RULES:
                    curr_frame = board_frame.assign_net_classes_window;
                    break;
                case PADSTACK_INFO:
                    curr_frame = board_frame.padstacks_window;
                    break;
                case PACKAGE_INFO:
                    curr_frame = board_frame.packages_window;
                    break;
                case COMPONENT_INFO:
                    curr_frame = board_frame.components_window;
                    break;
                case NET_INFO:
                    curr_frame = board_frame.net_info_window;
                    break;
                case INCOMPLETES_INFO:
                    curr_frame = board_frame.incompletes_window;
                    break;
    ...
    private boolean read_trace_colors(boolean p_fixed) throws IOException {
        ...
        board_handling.graphics_context.item_color_table.set_trace_colors(curr_colors, p_fixed);
        return true;
    }
    ...
    private boolean read_via_colors(boolean p_fixed) throws IOException {
        ...
        board_handling.graphics_context.item_color_table.set_via_colors(curr_colors, p_fixed);
        return true;
    }
    ...
    private Color[] read_color_array() throws IOException {
        ...
        Color[] result = color_list.toArray(Color[]::new);
        return result;
    }

    private void write_colors_scope() throws IOException {
        boardgraphics.GraphicsContext graphics_context = board_handling.graphics_context;
        out_file.start_scope();
    ...
    private void write_shove_enabled_scope() throws IOException {
        ...
        out_file.new_line();
        if (board_handling.settings.get_push_enabled()) {
            out_file.write("on");
        } else {
            out_file.write("off");
        }
    ...
    private void write_drag_components_enabled_scope() throws IOException {
        ...
        out_file.new_line();
        if (board_handling.settings.get_drag_components_enabled()) {
            out_file.write("on");
        } else {
            out_file.write("off");
        }
    ...
    private void write_ignore_conduction_scope() throws IOException {
        ...
        out_file.new_line();
        if (board_handling.get_routing_board().rules.get_ignore_conduction()) {
            out_file.write("on");
        } else {
            out_file.write("off");
        }
    ...
    private void write_automatic_layer_dimming_scope() throws IOException {
        ...
        out_file.new_line();
        Float layer_dimming = (float) board_handling.graphics_context.get_auto_layer_dim_factor();
    ...
    private void write_hilight_routing_obstacle_scope() throws IOException {
        ...
        out_file.new_line();
        if (board_handling.settings.get_hilight_routing_obstacle()) {
            out_file.write("on");
        } else {
            out_file.write("off");
        }
    ...
    private void write_via_snap_to_smd_center_scope() throws IOException {
        ...
        out_file.new_line();
        if (board_handling.settings.get_via_snap_to_smd_center()) {
            out_file.write("on");
        } else {
            out_file.write("off");
        }
