...
public class LocateFoundConnectionAlgo45Degree extends LocateFoundConnectionAlgo {
    ...
    protected Collection<FloatPoint> calculate_next_trace_corners() {
        ...
        int trace_halfwidth = ctrl.compensated_trace_half_width[current_trace_layer];
        ...
        if (!shrinked_room_shape.is_empty()) {
            ...
            boolean horizontal_first
                    = calc_horizontal_first_from_door(curr_from_info.door, current_from_point, nearest_room_point);
        ...
        if (current_to_door_index == current_target_door_index) {
            FloatPoint nearest_point = current_target_shape.nearest_point_approx(current_from_point);
            nearest_point = round_to_integer(nearest_point);
            FloatPoint add_corner = calculate_additional_corner(current_from_point, nearest_point, true, angle_restriction);
        ...
        nearest_to_door_point = round_to_integer(nearest_to_door_point);
        boolean horizontal_first
                = calc_horizontal_first_to_door(curr_to_info.door, current_from_point, nearest_to_door_point);
        result.add(calculate_additional_corner(current_from_point, nearest_to_door_point,
                horizontal_first, angle_restriction));
     ...
     * horizontal first when coming fromm p_from_point.
     */
