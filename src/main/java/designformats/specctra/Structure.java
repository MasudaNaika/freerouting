...
class Structure extends ScopeKeyword {
    ...
    private static void write_via_padstacks(library.BoardLibrary p_library, IndentFileWriter p_file,
            IdentifierType p_identifier_type) throws IOException {
        p_file.new_line();
        p_file.write("(via");
    ...
    private static void write_control_scope(rules.BoardRules p_rules, IndentFileWriter p_file) throws IOException {
        p_file.start_scope();
    ...
    private static void write_keepout_scope(WriteScopeParameter p_par, board.ObstacleArea p_keepout) throws IOException {
    ...
    static void write_snap_angle(IndentFileWriter p_file, board.AngleRestriction p_angle_restriction) throws IOException {
        p_file.start_scope();
    ...
    private boolean create_board(ReadScopeParameter p_par, BoardConstructionInfo p_board_construction_info) {
        ...
        PolylineShape[] outline_shape_arr = board_outline_shapes.toArray(PolylineShape[]::new);
    ...
    /**
     * Converts a dsn clearance rule into a board clearance rule. If p_layer_no
     * < 0, the rule is set on all layers. Returns true, if the string
     * smd_to_turn_gap was found.
     */
