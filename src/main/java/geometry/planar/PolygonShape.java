 ...
 * ordered in counterclock sense around the border of the shape. The corners are
 * normalysed, so that the corner with the lowest y-value comes first. In case
 * of equal y-value the corner with the lowest x-value comes first.
 *
 * @author Alfons Wirtz
 */
public class PolygonShape extends PolylineShape {
    ...
    /**
     * Splits this polygon shape into convex pieces. The result is not exact,
     * because rounded intersections of lines are used in the result pieces. It
     * can be made exact, if Polylines are returned instead of Polygons, so that
     * no intersection points are needed in the result.
     */
    @Override
    public TileShape[] split_to_convex() {
        if (precalculated_convex_pieces == null) // not yet precalculated
        {
    ...
    /**
     * Crivate recursive part of split_to_convex. Returns a collection of
     * polygon shape pieces.
     */
    ...
    private class DivisionPoint {
        ...
        DivisionPoint(int p_concave_corner_no) {
            ...
            for (int i = 0; i < loop_end; ++i) {
                ...
                if (corner_before_projection_approx.x != corner_after_projection_approx.x) // try a vertical division
                {
                    double min_x;
                    double max_x;
