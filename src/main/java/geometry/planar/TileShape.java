...
public abstract class TileShape extends PolylineShape implements ConvexShape, Serializable {
    ...
    public TileShape intersection_with_simplify(TileShape p_other) {
        TileShape result = intersection(p_other);
        return result.simplify();
    }
     ...
     * edge_line_count() - 1. The edge lines are sorted in counterclock sense
     * around the shape starting with the edge with the smallest direction.
     */
    @Override
    public abstract Line border_line(int p_no);

    /**
     * if p_line is a borderline of this shape the number of that edge is
    ...
    /**
     * Returns true, if p_point is not contained in the inside or the edge of
     * the shape
     */
    @Override
    ...
    /**
     * Returns true, if p_point is contained in this shape, but not on an edge
     * line
     */
    @Override
     ...
     * p_tolerance. p_tolerance is used when determing, if a point is on the
     * left side of a border line. It is used there in calculating a determinant
     * and is not the distance of p_point to the border.
     */
    ...
    /**
     * Returns true, if this shape contains p_other completely. THere may be
     * some numerical inaccurracy.
     */
    ...
    /**
     * Returns the distance between p_point and its nearest point on the shape.
     * 0, if p_point is contained in this shape
     */
    @Override
    ...
    /**
     * Returns the distance between p_point and its nearest point on the edge of
     * the shape.
     */
    @Override
    ...
    /**
     * Returns an approximation of the nearest point to p_from_point on the
     * border of the this shape
     */
    ...
    /**
     * Returns an approximation of the p_count nearest points to p_from_point on
     * the border of the this shape. The result points must be located on
     * different border lines and are sorted in ascending order (the nearest
     * point comes first).
     */
    ...
    public FloatLine diagonal_corner_segment() {
        ...
        FloatPoint first_corner = corner_approx(0);
        FloatPoint last_corner = corner_approx(border_line_count() / 2);
        return new FloatLine(first_corner, last_corner);
    }

    /**
     * Returns an approximation of the p_count nearest relative outside
     * locations of p_shape in the direction of different border lines of this
     * shape. These relative locations are sorted in ascending order (the
    ...
    public ConvexShape shrink(double p_offset) {
        ConvexShape result = offset(-p_offset);
        if (result.is_empty()) {
            IntBox centre_box = centre_of_gravity().bounding_box();
    ...
    /**
     * Calculates, if this Shape and p_other habe a common border piece and
     * returns an 2 dimensional array with the indices in this shape and p_other
     * of the touching edge lines in this case. Otherwise an array of dimension
     * 0 is returned. Used if the intersection shape is 1-dimensional.
     */
    public int[] touching_sides(TileShape p_other) {
        ...
        int side_no_1 = 0;
        Direction dir1 = border_line(0).direction();
     ...
     * p_line is on the left of this shape. Returns -1, if p_line is on the
     * right of this shape or intersects with the interiour of this shape.
     */
    ...
    /**
     * Returns Side.COLLINEAR, if p_line intersects with the interiour of this
     * shape, Side.ON_THE_LEFT, if this shape is completely on the left of
     * p_line or Side.ON_THE_RIGHT, if this shape is completely on the right of
     * p_line.
     */
    public Side side_of(Line p_line) {
        ...
        for (int i = 0; i < border_line_count(); ++i) {
            Side curr_side = p_line.side_of(corner(i));
            if (curr_side == Side.ON_THE_LEFT) {
                on_the_right = true;
     ...
     * p_from_point into the direction p_direction. p_from_point is assumed to
     * be inside this shape, otherwise -1 is returned.
     */
    public int intersecting_border_line_no(Point p_from_point, Direction p_direction) {
        ...
        for (int i = 0; i < border_line_count(); ++i) {
            Line curr_border_line = border_line(i);
    ...
    /**
     * Returns an arry of tuples of integers. The length of the array is the
     * number of points, where p_polyline enters or leaves the interiour of this
     * shape. The first coordinate of the tuple is the number of the line
     * segment of p_polyline, which enters the simplex and the second coordinate
     * of the tuple is the number of the edge_line of the simplex, which is
     * crossed there. That means that the entrance point is the intersection of
     * this 2 lines.
     */
    public int[][] entrance_points(Polyline p_polyline) {
        ...
        for (int line_no = 1; line_no < p_polyline.arr.length - 1; ++line_no) {
            ...
            for (int edge_no : curr_intersections) {
    ...
    /**
     * Cuts out the parts of p_polyline in the interiour of this shape and
     * returns a list of the remaining pieces of p_polyline. Pieces completely
     * contained in the border of this shape are not returned.
     */
    @Override
    public Polyline[] cutout(Polyline p_polyline) {
        ...
        if (intersection_no.length == 0) // no intersections
        {
            if (first_corner_is_inside) // p_polyline is contained completely in this shape
            {
        ...
        Point first_intersection
                = p_polyline.arr[curr_intersection_tuple[0]].intersection(border_line(curr_intersection_tuple[1]));
        if (!first_corner_is_inside) // calculate outside piece at start
        {
            if (!first_corner.equals(first_intersection)) // otherwise skip 1 point outside polyline at the start
            {
        ...
        while (curr_intersection_no < intersection_no.length - 1) // calculate the next outside polyline piece
        {
            ...
            for (int i = curr_intersection_no_of_polyline + 1; i < next_intersection_no_of_polyline; ++i) {
                if (is_outside(p_polyline.corner(i))) {
                    insert_piece = true;
                    break;
                }
            }
        ...
        if (curr_intersection_no <= intersection_no.length - 1) // calculate outside piece at end
        {
            curr_intersection_tuple = intersection_no[curr_intersection_no];
            int curr_polyline_intersection_no = curr_intersection_tuple[0];
        ...
        Polyline[] result = pieces.toArray(Polyline[]::new);
        return result;
    }
    ...
    public TileShape[] divide_into_sections(double p_max_section_width) {
        ...
        for (TileShape ts : section_boxes) {
            TileShape curr_section = intersection_with_simplify(ts);
            if (curr_section.dimension() == 2) {
                section_list.add(curr_section);
            }
        }
        TileShape[] result = section_list.toArray(TileShape[]::new);
        return result;
    }

    /**
     * Checks, if p_line_segment has a common point with the interiour of this
     * shape.
     */
    public boolean is_intersected_interiour_by(LineSegment p_line_segment) {
        ...
        Side[] border_line_side_of_start_point_arr = new Side[border_line_count()];
        ...
        for (Side side : border_line_side_of_start_point_arr) {
        ...
        for (Side side : border_line_side_of_end_point_arr) {
            if (side != Side.ON_THE_RIGHT) {
        ...
        for (int i = 0; i < border_line_side_of_start_point_arr.length; ++i) {
            ...
            if (border_line_side_of_start_point != border_line_side_of_end_point) {
                ...
                Side prev_corner_side = segment_line.side_of(corner_approx(i), 1);
                ...
                Side next_corner_side = segment_line.side_of(corner_approx(next_corner_index), 1);
    ...
    /**
     * Auxiliary function to implement the public function cutout(TileShape
     * p_shape)
     */
