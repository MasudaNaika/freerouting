...
/**
 * Contains internal auxiliary functions of class RoutingBoard for shoving
 * traces
 *
 * @author Alfons Wirtz
 */
public class ShoveTraceAlgo {
    ...
    /**
     * Checks if a shove with the input parameters is possible without clearance
     * violations p_dir is used internally to prevent the check from bouncing
     * back. Returns false, if the shove failed.
     */
    public boolean check(TileShape p_trace_shape, CalcFromSide p_from_side,
        ...
        if (!p_trace_shape.is_contained_in(board.get_bounding_box())) {
            board.set_shove_failing_obstacle(board.get_outline());
            return false;
        }
        ...
        if (!obstacles_shovable) {
            board.set_shove_failing_obstacle(shape_entries.get_found_obstacle());
            return false;
        }
        ...
        for (Via curr_shove_via : shape_entries.shove_via_list) {
            ...
            if (p_max_via_recursion_depth <= 0) {
                board.set_shove_failing_obstacle(curr_shove_via);
                return false;
            }
            ...
            for (int i = 0; i < try_via_centers.length; ++i) {
                if (i == 0 || curr_shove_via_center.distance_square(try_via_centers[i].to_float()) <= max_dist_square) {
                    ...
                    if (MoveDrillItemAlgo.check(curr_shove_via, delta, p_max_recursion_depth,
                            p_max_via_recursion_depth - 1, ignore_items, board, p_time_limit)) {
                        shove_via_ok = true;
                        break;
                    }
                }
    ...
    /**
     * Checks if a shove with the input parameters is possible without clearance
     * violations The result is the maximum lenght of a trace from the start of
     * the line segment to the end of the line segment, for wich the algoritm
     * succeedes. If the algorithm succeedes completely, the result will be
    ...
    /**
     * Puts in a trace segment with the input parameters and shoves obstacles
     * out of the way. If the shove does not work, the database may be damaged.
     * To prevent this, call check first.
     */
    public boolean insert(TileShape p_trace_shape, CalcFromSide p_from_side, int p_layer, int[] p_net_no_arr,
        ...
        if (!MoveDrillItemAlgo.shove_vias(p_trace_shape, p_from_side, p_layer, p_net_no_arr, p_cl_type,
                p_ignore_items, p_max_recursion_depth, p_max_via_recursion_depth, true, board)) {
            return false;
        }
        ShapeTraceEntries shape_entries
                = new ShapeTraceEntries(p_trace_shape, p_layer, p_net_no_arr, p_cl_type, p_from_side, board);
        ...
        if (!shape_entries.shove_via_list.isEmpty()) {
            ...
            board.set_shove_failing_obstacle(shape_entries.shove_via_list.iterator().next());
            return false;
        }
        if (!obstacles_shovable) {
            board.set_shove_failing_obstacle(shape_entries.get_found_obstacle());
    ...
    /**
     * Checks, if there are obstacle in the way of p_polyline and tries to wrap
     * the polyline trace around these obstacles in counterclock sense. Returns
     * null, if that is not possible. Returns p_polyline, if there were no
     * obstacles If p_contact_pins != null, all pins not contained in
     * p_contact_pins are regarded as obstacles, even if they are of the own
     * net.
     */
    private Polyline spring_over(Polyline p_polyline, int p_half_width,
        ...
        if (p_recursion_depth <= 0 || found_obstacle instanceof BoardOutline || (found_obstacle instanceof Trace && !found_obstacle.is_shove_fixed())) {
            board.set_shove_failing_obstacle(found_obstacle);
            return null;
        }
    ...
    /**
     * Checks, if there are obstacle in the way of p_polyline and tries to wrap
     * the polyline trace around these obstacles. Returns null, if that is not
     * possible. Returns p_polyline, if there were no obstacles This function
     * looks contrary to the previous function for the shortest way around the
     * obstaccles. If p_contact_pins != null, all pins not contained in
     * p_contact_pins are regarded as obstacles, even if they are of the own
     * net.
     */
