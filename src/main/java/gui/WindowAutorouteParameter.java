...
public class WindowAutorouteParameter extends BoardSavableSubWindow {
    ...
    public WindowAutorouteParameter(BoardFrame p_board_frame) {
        ...
        for (int i = 0; i < signal_layer_count; ++i) {
            ...
            combo_box_arr[i].addItem(horizontal);
            combo_box_arr[i].addItem(vertical);
    ...
    public void refresh() {
        ...
        vias_allowed.setSelected(settings.get_vias_allowed());
        ...
        for (int i = 0; i < signal_layer_active_arr.length; ++i) {
            signal_layer_active_arr[i].setSelected(settings.get_layer_active(layer_structure.get_layer_no(i)));
        }

        for (int i = 0; i < combo_box_arr.length; ++i) {
            if (settings.get_preferred_direction_is_horizontal(layer_structure.get_layer_no(i))) {
                combo_box_arr[i].setSelectedItem(horizontal);
            } else {
                combo_box_arr[i].setSelectedItem(vertical);
            }
        }
    ...
    private class LayerActiveListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent p_evt) {
            ...
            board_handling.settings.autoroute_settings.set_layer_active(curr_layer_no, signal_layer_active_arr[signal_layer_no].isSelected());
        }
