 ...
 * of the packege pins, and optional other stuff like an outline package
 * keepouts.
 *
 * @author alfons
 */
public class Package implements Comparable<Package>, board.ObjectInfoPanel.Printable, Serializable {
    ...
    /**
     * Compares 2 packages by name. Useful for example to display packages in
     * alphabetic order.
     */
    @Override
    ...
    /**
     * Returns the pin number of the pin with the input name from this package,
     * or -1, if no such pin exists Pin numbers are from 0 to pin_count - 1.
     */
    ...
    public void print_info(board.ObjectInfoPanel p_window, Locale p_locale) {
        ...
        for (Pin curr_pin : pin_arr) {
            ...
            Padstack curr_padstack = package_list.padstack_list.get(curr_pin.padstack_no);
    ...
    public static class Pin implements Serializable {
        ...
        /**
         * Creates a new package pin with the input coordinates relative to the
         * package location.
         */
